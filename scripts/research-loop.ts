...
function recordResult(result: ExperimentResult): void {
	const header =
		"timestamp\texperiment\tsystem\ttokens_used\ttokens_saved\texecution_time\tsuccess\tnotes\n";
...
async function runCompetitionRound(): Promise<void> {
	...
	for (const experiment of EXPERIMENTS) {
		...
		log(
			`8gent: ${result8gent.tokensUsed} tokens | Claude: ${resultClaude.tokensUsed} tokens | Savings: ${savings}%`,
		);
	}
