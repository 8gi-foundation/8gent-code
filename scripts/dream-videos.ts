...
// Load env vars: OPENROUTER_API_KEY from .env, FAL_KEY from [REDACTED-PATH]/.env.local
import { config as dotenvConfig } from "dotenv";
dotenvConfig({ path: path.join(os.homedir(), "8gent-code", ".env") });
dotenvConfig({
	path: path.join(os.homedir(), "personal-os", ".env.local"),
	override: false,
});
...
if (!FAL_KEY) {
	console.error("FAL_KEY not found in env. Set it in [REDACTED-PATH]/.env.local");
	process.exit(1);
}
...
async function generateVideo(prompt: string): Promise<string | null> {
	...
	try {
		...
		const response = await fetch(
			"https://queue.fal.run/fal-ai/kling-video/v2.1/master/text-to-video",
			{
				method: "POST",
				headers: {
