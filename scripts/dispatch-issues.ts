...
async function executeTask(
	...
	// Create session
	const sessionReply = await sendAndWait(
		ws,
		{ type: "session:create", channel: "delegation" },
		"session:created",
		15_000,
	);
		...
		const handler = (event: MessageEvent) => {
			...
			switch (evt) {
				...
				case "tool:start":
					toolCalls++;
					console.log(
						`  [tool] ${payload.tool}(${JSON.stringify(payload.input || {}).slice(0, 80)})`,
					);
					break;
				case "tool:result":
					console.log(
						`  [result] ${payload.tool} (${payload.durationMs}ms) ${(payload.output || "").slice(0, 60)}`,
					);
					break;
				case "agent:stream":
					if (payload.final) {
