...
const content = readFileSync(
	join(import.meta.dir, "../docs/content/2026-03-20-day1-launch.md"),
	"utf-8",
);
...
const linkedin =
	sections
		.find((s) => s.includes("## LinkedIn"))
		?.replace("## LinkedIn", "")
		.trim() || "";
const twitter =
	sections
		.find((s) => s.includes("## X / Twitter"))
		?.replace("## X / Twitter", "")
		.trim() || "";
const threads =
	sections
		.find((s) => s.includes("## Threads"))
		?.replace("## Threads / Instagram", "")
		.trim() || "";
const artem =
	sections
		.find((s) => s.includes("## Artem"))
		?.replace("## Artem Luko Reply (LinkedIn - AI Perks)", "")
		.trim() || "";
