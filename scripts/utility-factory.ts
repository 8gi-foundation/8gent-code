...
function securityReview(code: string, name: string): string[] {
	...
	for (const line of lines) {
		...
		if (
			/^(export|import|type |interface |const |let |var |function |class |enum |declare |\/\/)/.test(
				trimmed,
			)
		)
			continue;
		...
		if (
			/^(if|else|for|while|switch|case|break|return|throw|try|catch|finally|default|do)/.test(
				trimmed,
			)
		)
			continue; // control flow inside functions
	...
	if (
		/child_process|exec\s*\(|execSync|spawn/.test(code) &&
	...
	if (
		/\bfetch\s*\(|http\.|https\.|net\.|dgram\./.test(code) &&
		!name.includes("http") &&
		!name.includes("url") &&
		!name.includes("fetch") &&
		!name.includes("net")
...
async function sendTelegramVoice(audioPath: string): Promise<void> {
	...
	try {
		...
		form.append(
			"voice",
			new Blob([fs.readFileSync(audioPath)], { type: "audio/ogg" }),
			"voice.ogg",
		);
...
async function main() {
	...
	for (const spec of pending) {
		...
		try {
				...
				appendLog(
					{
						name: spec.name,
						status: "error",
						error: `security: ${securityIssues.join("; ")}`,
						timestamp: new Date().toISOString(),
					},
					log,
				);
			...
			try {
				execSync(
					`bun build "${tmpValidatePath}" --no-bundle --outdir /tmp/.8gent-factory-out 2>&1`,
					{
						encoding: "utf-8",
						timeout: 15000,
					},
			...
			execSync(`git checkout main && git checkout -b ${branch}`, {
				cwd: ROOT,
				stdio: "pipe",
			});
			...
			// Commit
			execSync(`git add "${toolPath}" "${quarantinePath}"`, {
				cwd: ROOT,
				stdio: "pipe",
			});
			execSync(
