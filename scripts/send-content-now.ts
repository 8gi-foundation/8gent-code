...
const content = readFileSync(
	join(
		os.default.homedir(),
		"personal-os/content/daily-posts/2026-03-20/2026-03-20-day1-launch.md",
	),
	"utf-8",
);
...
const linkedin =
	parts
		.find((s) => s.startsWith("## LinkedIn"))
		?.replace("## LinkedIn", "")
		.trim() || "";
const twitter =
	parts
		.find((s) => s.startsWith("## X / Twitter"))
		?.replace("## X / Twitter", "")
		.trim() || "";
const threads =
	parts
		.find((s) => s.startsWith("## Threads / Instagram"))
		?.replace("## Threads / Instagram", "")
		.trim() || "";
const artem =
	parts
		.find((s) => s.startsWith("## Artem"))
		?.replace(/## Artem.*/, "")
		.trim() || "";
