	...
	return new Promise((resolve) => {
		const deadline = setTimeout(() => {
			...
			resolve({
				model,
				status: "timeout",
				response: chunks.join("").slice(0, 500),
				toolCalls,
			...
			if (evt === "tool:start") {
				...
				if (payload.final) {
					...
					resolve({
						model,
						status: "success",
						response: (payload.chunk || chunks.join("")).slice(0, 500),
						toolCalls,
						durationMs: Date.now() - startMs,
						wroteFile,
						ranTest,
...
async function main() {
		...
		try {
			const proc = Bun.spawn(
				[
					...
					"eight-vessel",
					"-C",
					"bash -c 'rm -f /app/packages/memory/shootout-test.ts'",
				],
	...
	console.log("=".repeat(70));
	console.log(
		`${"Model".padEnd(35)} ${"Status".padEnd(8)} ${"Wrote".padEnd(6)} ${"Ran".padEnd(5)} ${"Pass".padEnd(6)} ${"Tools".padEnd(6)} ${"Time"}`,
	);
	console.log("─".repeat(70));
	...
	if (winners.length > 0) {
		...
		console.log(
			`Fastest winner: ${fastest.model.split("/")[1]} in ${Math.round(fastest.durationMs / 1000)}s`,
		);
	}
	console.log("=".repeat(70));
