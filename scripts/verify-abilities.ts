...
function sendAndWait(
	...
	return new Promise((resolve, reject) => {
		...
		const timer = setTimeout(
			() => reject(new Error(`Timeout waiting for response to: ${prompt}`)),
			timeoutMs,
		);
...
async function run(): Promise<void> {
	...
	try {
		const recallResp = await sendAndWait(
			ws,
			sessionId,
			"Recall any facts about 'verification script'",
		);
		const recalled =
			recallResp.toLowerCase().includes("verification") ||
	...
	try {
		const evolResp = await sendAndWait(
			ws,
			sessionId,
