...
const maxFindings = parseInt(args.find((_, i, a) => a[i - 1] === "--max") || "20");
...
const RELEVANCE_KEYWORDS = [
  // AI agent patterns
  "agent framework", "multi-agent", "tool use", "function calling",
  "agentic", "agent loop", "agent orchestration", "agent sandbox",
  "code agent", "coding agent", "autonomous agent",
  // Infrastructure patterns
  "harness", "sandbox", "isolation", "worktree", "spawn",
  "message bus", "event sourcing", "audit log", "append-only",
  // Model/inference
  "fine-tune", "lora", "qlora", "model serving", "inference",
  "ollama", "llama.cpp", "vllm", "openrouter", "groq",
  // Developer tools
  "cli tool", "developer tool", "code review", "ast",
  "language server", "lsp", "tree-sitter", "syntax",
  // Security
  "prompt injection", "jailbreak", "red team", "ai safety",
  "content policy", "guardrails", "alignment",
  // Product patterns
  "onboarding", "retention", "activation", "growth loop",
  "right friction", "progressive disclosure",
  // Specific tech
  "bun", "deno", "next.js", "vercel", "fly.io", "cloudflare workers",
  "websocket", "webrtc", "service worker", "pwa",
  // AAC / Accessibility (for 8gent Jr)
  "aac", "augmentative communication", "accessibility", "a11y",
  "assistive technology", "coppa", "child safety",
  // Ecosystem
  "mcp", "model context protocol", "claude code", "cursor",
  "windsurf", "cline", "open source ai",
];
...
`;
        ...
        const posts = JSON.parse(result.stdout.trim() || "[]") as Array<{
          text: string; author: string; links: string[];
        }>;
...
*${new Date().toISOString()}*`;
  ...
  try {
    ...
    if (result.status === 0) {
      ...
      const retryResult = spawnSync("gh", [
        "issue", "create",
        "--repo", repo,
        "--title", `research: ${finding.title}`,
        "--body", body,
...
export async function runSocialResearch(): Promise<{ findings: Finding[]; issues: string[] }> {
  log("═══════════════════════════════════════════════════");
