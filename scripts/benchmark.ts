...
async function runFullBenchmark(): Promise<BenchmarkSuite> {
	...
	log(
		`║  Traditional tokens:    ${totalTraditional.toLocaleString().padStart(14)}                    ║`,
		colors.green,
	);
	log(
		`║  AST-first tokens:      ${totalAst.toLocaleString().padStart(14)}                    ║`,
		colors.green,
	);
	log(
		`║  Total tokens saved:    ${(totalTraditional - totalAst).toLocaleString().padStart(14)}                    ║`,
		colors.green,
	);
	log(
...
async function main() {
	...
	if (args.includes("--symbols")) {
		...
		log(
			`   Saved:       ${result.savings.tokens.toLocaleString()} tokens (${result.savings.percent.toFixed(1)}%)`,
		);
	} else {
