...
function logCheckpointResume(data: RunCheckpoint): void {
	log(
		`[RESUME] Found checkpoint: iter ${data.iteration}, task ${data.taskIndex}/${(data.results as unknown[]).length} completed`,
	);
}
...
function loadHarnessState(): void {
	try {
		if (fs.existsSync(HARNESS_STATE_PATH)) {
			...
			log(
				`[RESUME] Loaded harness state: temp=${harnessState.globalTemperature}, steps=${harnessState.maxSteps}`,
			);
		}
	} catch {
		/* start fresh */
...
async function runBenchmarks(model: string): Promise<BenchmarkResult[]> {
	...
	const tasks = [
		{
			id: "ARC-pattern-induction",
			prompt: `Given these input-output pairs, discover the transformation rule and apply it to the test input.
...
Write a TypeScript function that implements the discovered rule generically (not hardcoded for these examples), apply it to the test input, and verify your answer with a test. Explain your reasoning step by step before writing code.`,
		...
		{
			id: "ARC-grid-abstraction",
			prompt: `You are given a 5x5 grid. The rule transforms it based on a pattern you must discover.
...
Discover the rule. Write TypeScript that implements it generically. Show your reasoning. Verify with tests.`,
		...
		{
			id: "novel-algorithm",
			prompt: `Design and implement a data structure called IntervalMerger that supports these operations in better than O(n) per operation:
...
The catch: intervals can overlap, be adjacent, or be nested. Your merge operation must handle all cases. Write implementation + comprehensive tests. Analyze the time complexity of each operation.`,
...

		{
			...
			prompt: `Build a rule engine in TypeScript where:
...
This tests your ability to reason about recursive self-modifying systems.`,
...

		{
			...
			prompt: `Implement a cellular automaton in TypeScript with these requirements:
...
Initialize with a 10x10 random grid seeded with seed=42 for reproducibility.`,
	...
	if (checkpoint && checkpoint.iteration === 0) {
		// iteration 0 = current runBenchmarks call
	...
	for (let ti = startIndex; ti < tasks.length; ti++) {
		...
		try {
			...
			const result = await runCommand(
				...
				[
					"run",
					path.join(PROJECT_ROOT, "packages/harness-cli/index.ts"),
					"run",
					effectivePrompt,
					"--model",
					model,
...
Focus on getting working code with passing tests. Simplify if needed.`;
				...
				const retryResult = await runCommand(
					...
					[
						...
						runtime,
						"--max-steps",
						String(Math.min(30, maxStepsForMode + 5)),
						"--timeout",
						String(taskTimeout),
						"--json",
					],
	...
	log(
		`Benchmark summary: avg=${avgScore.toFixed(1)}, pass=${(passRate * 100).toFixed(0)}% (${results.filter((r) => r.passed).length}/${results.length})`,
	);
...
async function analyzeAndMutate(
	...
	for (const fail of failures) {
					...
					try {
						...
						if (
							...
							if (typeof errorMsg === "string" && errorMsg.length > 10) {
								mutations.push(
									`// NIGHTLY LEARNING (iter ${iteration}, benchmark ${fail.benchmarkId}): ${errorMsg.slice(0, 200)}`,
								);
							}
						}
					} catch {
		...
		mutations.push(
			`// BENCHMARK FAILURE ${fail.benchmarkId}: Score ${fail.score}. Needs improvement.`,
		);
	}
...
async function postToBoardroom(
	...
	try {
		const res = await fetch(
			`https://discord.com/api/v10/channels/${DISCORD_BOARDROOM_CHANNEL}/messages`,
			{
				method: "POST",
				headers: {
...
function selfImprove(results: BenchmarkResult[], iteration: number): void {
	...
	if (passRate < 0.4 && harnessState.maxSteps < 30) {
		...
		log(
			`  [HYPER] Low pass rate with rich analysis - increasing max steps to ${harnessState.maxSteps}`,
		);
...
async function trainLoRA(
	...
	const result = await runCommand(
		...
		[
			...
			dataPath,
			"--base-model",
			"Qwen/Qwen2.5-Coder-14B", // HuggingFace model ID
			"--output",
			outputDir,
			"--epochs",
			"1",
		...
		{ timeout: 600000, cwd: PROJECT_ROOT },
...
async function main() {
	...
	try {
		...
		log(
			`Social research: ${research.findings.length} findings, ${research.issues.length} issues created`,
		);
	} catch (err) {
	...
	for (const { iteration, results, model } of allResults) {
		...
		log(
			`Iteration ${iteration} (${model}): avg=${avg.toFixed(1)}, pass=${passed}/${results.length}`,
		);
	}
