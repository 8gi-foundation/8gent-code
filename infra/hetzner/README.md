...
Runs the Eight daemon and two Telegram bridges (`@aijamesosbot` and
`@eightgentcodebot`) on the Hetzner cax21 box at `78.47.98.218`. Each bridge
...
|------------|-----------|----------|--------------|
| `aijames` (default) | `TELEGRAM_BOT_TOKEN` | `TELEGRAM_CHAT_ID` | `@aijamesosbot` |
...
ssh root@78.47.98.218 'echo ok'

# 2. From your laptop, push the repo:
bash infra/hetzner/deploy.sh 78.47.98.218 sync

# 3. From your laptop, run bootstrap on the box:
ssh root@78.47.98.218 'bash /opt/8gent/infra/hetzner/bootstrap.sh'

# 4. On the box, fill in real secrets:
ssh root@78.47.98.218
...
bash infra/hetzner/deploy.sh 78.47.98.218 up
```

## Day-to-day

```bash
bash infra/hetzner/deploy.sh 78.47.98.218 sync     # push code, no restart
bash infra/hetzner/deploy.sh 78.47.98.218 up       # push + rebuild + start
bash infra/hetzner/deploy.sh 78.47.98.218 restart
bash infra/hetzner/deploy.sh 78.47.98.218 logs
bash infra/hetzner/deploy.sh 78.47.98.218 status
bash infra/hetzner/deploy.sh 78.47.98.218 down
```
...
ssh root@78.47.98.218 'curl -fsS http://127.0.0.1:18789/health'

# Bridge logs:
ssh root@78.47.98.218 'docker logs --tail=50 eight-bridge-aijames'
ssh root@78.47.98.218 'docker logs --tail=50 eight-bridge-eightgent'
```
