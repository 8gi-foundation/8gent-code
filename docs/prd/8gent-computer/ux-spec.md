...
- **Theme inheritance, not Match System.** Default theme is inherited from 8gentOS (`8gi-foundation/8gent-OS`) and the AI James OS parent (`[REDACTED-PATH]/`), governed by `BRAND.md`. Default on first launch is **dark** (8gentOS developer default), not the macOS system appearance. Tokens, typography, palette, and component grammar pull from the same source-of-truth as 8gentOS. The light/dark toggle still exists and persists. Detail in section 3.5.
