...
The Hetzner dedicated server at `[REDACTED-IP]` is the first production vessel in the 8GI fleet, hosting `james.8gentos.com`.
...
```bash
ssh -i ~/.ssh/hetzner_8gi_ed25519 james@[REDACTED-IP]
```
...
```bash
ssh james@[REDACTED-IP] systemctl status 8gent-os.service
...
```bash
ssh james@[REDACTED-IP] sudo systemctl stop 8gent-os.service
# fix the issue in /opt/8gent-OS, then:
ssh james@[REDACTED-IP] sudo systemctl start 8gent-os.service
```
