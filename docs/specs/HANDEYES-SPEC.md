...
3. **DoomLoopDetector hook contract.** The detector currently exposes a synchronous detection result; it does not emit an event stream. Either we add an event emitter to it (small surface change to `tool-loop-detector.ts`) or the coordinator polls. Polling is uglier but keeps the detector untouched. RFC at engagement-loop PR time.
