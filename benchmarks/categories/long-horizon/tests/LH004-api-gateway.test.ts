...
beforeEach(async () => {
	try {
		...
		try {
			router = await import(path.join(WORK_DIR, "router.js"));
		} catch {}
	}
	try {
		...
		try {
			middleware = await import(path.join(WORK_DIR, "middleware.js"));
		} catch {}
	}
	try {
		...
		try {
			cache = await import(path.join(WORK_DIR, "cache.js"));
		} catch {}
	}
	try {
		...
		try {
			transformer = await import(path.join(WORK_DIR, "transformer.js"));
		} catch {}
	}
	try {
		...
		try {
			gateway = await import(path.join(WORK_DIR, "gateway.js"));
		} catch {}
	}
});
...
describe("CORS Middleware", () => {
	it("corsMiddleware sets CORS headers", async () => {
		...
		const hasCorsHeader = headerKeys.some(
			(k) => k.includes("access-control") || k.includes("cors"),
		);
...
describe("RequestTransformer", () => {
	it("adds and removes headers", () => {
		const RT =
			...
			transformer.default?.RequestTransformer ||
			transformer.default;
		const t = new RT();
		const transformed = t
