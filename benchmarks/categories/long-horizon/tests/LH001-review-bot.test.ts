...
beforeEach(async () => {
	try {
		...
		try {
			diffParser = await import(path.join(WORK_DIR, "diff-parser.js"));
		} catch {}
	}
	try {
		...
		try {
			astAnalyzer = await import(path.join(WORK_DIR, "ast-analyzer.js"));
		} catch {}
	}
	try {
	...
	try {
		...
		try {
			styleChecker = await import(path.join(WORK_DIR, "style-checker.js"));
		} catch {}
	}
	try {
...
`;
		...
		const evalIssues = issues.filter(
			(i: any) =>
...
describe("Style Checker", () => {
	it("checkStyle detects lines too long", () => {
		...
		const longViolations = violations.filter(
			(v: any) =>
