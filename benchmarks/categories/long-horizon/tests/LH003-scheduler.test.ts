...
beforeEach(async () => {
	try {
		...
		try {
			cronParser = await import(path.join(WORK_DIR, "cron-parser.js"));
		} catch {}
	}
	try {
		...
		try {
			priorityQueue = await import(path.join(WORK_DIR, "priority-queue.js"));
		} catch {}
	}
	try {
		...
		try {
			workerPool = await import(path.join(WORK_DIR, "worker-pool.js"));
		} catch {}
	}
	try {
		...
		try {
			circuitBreaker = await import(path.join(WORK_DIR, "circuit-breaker.js"));
		} catch {}
	}
	try {
		...
		try {
			scheduler = await import(path.join(WORK_DIR, "scheduler.js"));
		} catch {}
	}
});
...
describe("CircuitBreaker", () => {
	it("transitions closed → open on failures", async () => {
		const CB =
			...
			circuitBreaker.default?.CircuitBreaker ||
