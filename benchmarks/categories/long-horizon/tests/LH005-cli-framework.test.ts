...
beforeEach(async () => {
	try {
		...
		try {
			argParser = await import(path.join(WORK_DIR, "arg-parser.js"));
		} catch {}
	}
	try {
	...
	try {
		...
		try {
			helpGenerator = await import(path.join(WORK_DIR, "help-generator.js"));
		} catch {}
	}
	try {
		...
		try {
			progress = await import(path.join(WORK_DIR, "progress.js"));
		} catch {}
	}
	try {
		...
		try {
			formatter = await import(path.join(WORK_DIR, "formatter.js"));
		} catch {}
	}
});
...
describe("Arg Parser", () => {
	it("handles --key=value", () => {
		...
		const defs = [{ name: "name", type: "string" }];
	...
	it("handles --flag (boolean)", () => {
		const fn = argParser.parseArgs || argParser.default?.parseArgs;
		const defs = [{ name: "verbose", type: "boolean" }];
	...
	it("handles -f alias", () => {
		const fn = argParser.parseArgs || argParser.default?.parseArgs;
		const defs = [{ name: "force", alias: "f", type: "boolean" }];
	...
	it("handles --no-flag negation", () => {
		const fn = argParser.parseArgs || argParser.default?.parseArgs;
		const defs = [{ name: "color", type: "boolean", default: true }];
	...
	it("handles array args (--include a --include b)", () => {
		const fn = argParser.parseArgs || argParser.default?.parseArgs;
		const defs = [{ name: "include", type: "array" }];
	...
	it("validates required args", () => {
		const fn = argParser.parseArgs || argParser.default?.parseArgs;
		const defs = [{ name: "name", type: "string", required: true }];
...
describe("CommandRegistry", () => {
	it("dispatches to correct handler", async () => {
		const CR =
			...
			commandRegistry.default?.CommandRegistry ||
			...
			subcommands: [
				{
					...
					handler: (args: any) => {
						result = `created:${args.args?.name || args.name || "unnamed"}`;
					},
				},
			],
			handler: () => {},
