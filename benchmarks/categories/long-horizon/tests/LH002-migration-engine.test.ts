...
beforeEach(async () => {
	try {
		...
		try {
			schema = await import(path.join(WORK_DIR, "schema.js"));
		} catch {}
	}
	try {
		...
		try {
			migration = await import(path.join(WORK_DIR, "migration.js"));
		} catch {}
	}
	try {
		...
		try {
			executor = await import(path.join(WORK_DIR, "executor.js"));
		} catch {}
	}
	try {
		...
		try {
			history = await import(path.join(WORK_DIR, "history.js"));
		} catch {}
	}
});
...
describe("Schema Diffing", () => {
	it("diffSchemas detects added tables", () => {
		...
		const addedNames = (diff.tablesAdded || []).map((t: any) =>
	...
	it("diffSchemas detects removed tables", () => {
		...
		const removedNames = (diff.tablesRemoved || []).map((t: any) =>
			typeof t === "string" ? t : t.name,
		);
		...
		const migrations = [
			{
				...
				name: "first",
				timestamp: 1,
				up: ["CREATE TABLE a", "CREATE INDEX idx ON a(id)"],
				down: ["DROP TABLE a"],
				checksum: "a",
			},
		];
