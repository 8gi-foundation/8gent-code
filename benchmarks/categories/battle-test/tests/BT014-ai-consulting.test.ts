...
describe("Assessment", () => {
	it("assessAIReadiness returns a score between 0 and 100", () => {
		const assessAIReadiness =
			assessment.assessAIReadiness || assessment.default?.assessAIReadiness;
	...
	it("benchmarkAgainstIndustry returns percentile", () => {
		const benchmarkAgainstIndustry =
			assessment.benchmarkAgainstIndustry ||
			assessment.default?.benchmarkAgainstIndustry;
		const industryData = [
...
describe("Recommendation", () => {
	it("generateRecommendations returns array of recommendations", () => {
		...
		const generateRecommendations =
			...
			recommendation.default?.generateRecommendations;
		const report = assessAIReadiness(sampleProfile);
	...
	it("prioritize by roi sorts descending by estimatedROI", () => {
		const prioritize =
			recommendation.prioritize || recommendation.default?.prioritize;
		const recs = [
			{
				title: "A",
				impact: "low",
	...
	it("estimateTotalCost returns cost, timeline, expectedROI", () => {
		const estimateTotalCost =
			recommendation.estimateTotalCost ||
			recommendation.default?.estimateTotalCost;
		const recs = [
			{
				title: "A",
				timelineWeeks: 4,
...
describe("Roadmap", () => {
	it("generateRoadmap returns array of phases", () => {
		const generateRoadmap =
			roadmap.generateRoadmap || roadmap.default?.generateRoadmap;
		const recs = [
			{
				title: "Data Cleanup",
				timelineWeeks: 4,
