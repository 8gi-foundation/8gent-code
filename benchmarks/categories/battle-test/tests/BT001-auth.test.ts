...
beforeEach(async () => {
	try {
		...
		// Try alternate names
		try {
			auth = await import(path.join(WORK_DIR, "auth.js"));
		} catch {}
		try {
			rbac = await import(path.join(WORK_DIR, "rbac.js"));
		} catch {}
		try {
			rateLimiter = await import(path.join(WORK_DIR, "rate-limiter.js"));
		} catch {}
		try {
			userStore = await import(path.join(WORK_DIR, "user-store.js"));
		} catch {}
	}
});
