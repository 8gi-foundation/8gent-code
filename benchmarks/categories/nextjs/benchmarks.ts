/**
 * 8gent Code Benchmarks - Next.js Web Development
 *
...
5. Types the fetched data properly`,
		...
		rubric: {
			...
			bestPractices: {
				weight: 15,
				criteria: ["Uses loading.tsx", "Uses error.tsx", "Proper metadata"],
			},
		},
		validators: [
			{
				type: "ast",
				config: { mustContain: ["Suspense", "async", "revalidate"] },
			},
		],
		expectedTokens: 900,
		timeLimit: 150,
...
6. Prevents double submission`,
		...
		rubric: {
			...
			bestPractices: {
				weight: 15,
				criteria: ["Uses useFormState", "Accessible form", "CSRF protection"],
			},
		},
		validators: [
			{
				type: "ast",
				config: { mustContain: ["use server", "useFormStatus", "z.object"] },
			},
		],
		expectedTokens: 1000,
		timeLimit: 180,
		...
		validators: [
			{
				type: "ast",
				config: {
					mustContain: ["middleware", "NextRequest", "cookies", "redirect"],
				},
			},
		],
		expectedTokens: 800,
