...
function getEightSystemPrompt(): string {
	...
	try {
		...
		cachedSystemPrompt =
			"You are Eight, the infinite gentleman — an autonomous coding agent. Output clean, production-quality TypeScript. Be concise.";
	}
...
async function runClaude(task: SWETask): Promise<{ output: string; durationMs: number }> {
				...
				messages: [
					{
						role: "system",
						content:
							"You are Claude, an expert software engineer. Output clean, production-quality TypeScript. Be concise.",
					},
					{ role: "user", content: fullPrompt },
				],
				stream: false,
	...
	const res = await fetch("https://api.anthropic.com/v1/messages", {
		...
		body: JSON.stringify({
			...
			system:
				"You are an expert software engineer. Output clean, production-quality TypeScript. Be concise. Output ONLY code.",
		}),
	});
...
{"scoreA": <number>, "scoreB": <number>, "analysisA": "<one sentence>", "analysisB": "<one sentence>", "winner": "A" | "B" | "tie"}`;
	...
	try {
		...
		return {
			eightScore: 50,
			claudeScore: 50,
			eightAnalysis: "Judge parse failed",
			claudeAnalysis: "Judge parse failed",
			winner: "tie",
		};
	}
}
...
async function main() {
	...
	fs.writeFileSync(
		...
		JSON.stringify(
			{
				results,
				summary: { eightWins, claudeWins, ties, eightAvg, claudeAvg },
			},
			null,
			2,
		),
