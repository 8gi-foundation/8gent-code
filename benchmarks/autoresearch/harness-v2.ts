#!/usr/bin/env bun
/**
 * harness-v2.ts — 8gent Benchmark v2
 *
...
async function runBenchmark(
	...
	const {
		...
		durationMs: apiMs,
		tokenUsage,
	} = await callWithFallback(messages, temperature);
...
async function main(): Promise<void> {
	...
	log("═══════════════════════════════════════════════════════════════");
	log("  8gent Benchmark v2 — OpenRouter + Execution Grading");
	log("═══════════════════════════════════════════════════════════════");
			...
			results.push({
				...
				grade: {
					...
					keyword: {
						score: 0,
						matchedKeywords: [],
						missedKeywords: benchmark.keywords,
					},
					method: "keyword-only",
				},
				timestamp: Date.now(),
	...
	log(
		`  Avg tokens/benchmark: ${results.length > 0 ? Math.round(totalTokens / results.length) : 0}`,
	);
	log(
		`  Avg time/benchmark:   ${results.length > 0 ? (totalTime / results.length / 1000).toFixed(1) : 0}s`,
	);
	log(
		`  Score/1k tokens:      ${totalTokens > 0 ? (avgScore / (totalTokens / 1000)).toFixed(2) : "n/a"}`,
	);
