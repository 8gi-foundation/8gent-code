...
async function main(): Promise<void> {
	...
	for (let iter = state.iteration; iter < MAX_ITERATIONS; iter++) {
		...
		log(
			`  Tokens used:   ${iterTokens} (avg ${Math.round(iterTokens / benchmarks.length)}/benchmark)`,
		);
		log(
			`  Time taken:    ${(iterDuration / 1000).toFixed(1)}s (avg ${(iterDuration / benchmarks.length / 1000).toFixed(1)}s/benchmark)`,
		);
		log(
			`  Efficiency:    ${iterTokens > 0 ? (avgScore / (iterTokens / 1000)).toFixed(2) : "n/a"} score/1k tokens`,
		);
