...
describe("UI006 - Dark Theme Analytics Dashboard", () => {
	...
	test("bar chart with 7+ child elements with varying heights", () => {
		...
		expect(
			Math.max(barElements.length, inlineHeights.length, barClasses.length),
		).toBeGreaterThanOrEqual(7);
	});

	test("sidebar nav with 4+ navigation items", () => {
		const navItems =
			html.match(/<(a|li|button)[^>]*class="[^"]*nav[^"]*"[^>]*>/gi) ??
	...
	test("green/red colors for positive/negative indicators", () => {
		const hasGreen =
			css.match(
				/(#[0-9a-f]*[2-9a-f][0-9a-f]*[0-4][0-9a-f]|green|#[24][0-9a-f]b|rgb\([^)]*,\s*(1[2-9]\d|[2-9]\d\d)\s*,)/i,
			) || html.match(/(green|positive|success|up)/i);
		const hasRed =
			css.match(
				/(#[ef][0-9a-f][0-4]|red|#f[0-9a-f][0-4]|rgb\(\s*(1[5-9]\d|[2-9]\d\d)\s*,\s*[0-6]\d)/i,
	...
	test("text colors are light enough for contrast (no pure black text on dark bg)", () => {
		const hasLightText =
			css.match(
				/color\s*:\s*(#[a-f][a-f0-9]{5}|#[c-f][0-9a-f]{2,5}|white|#fff|rgba?\(\s*(1[5-9]\d|[2-9]\d\d))/i,
			) ||
			css.match(
				/--[\w-]*(text|color|fg)[\w-]*\s*:\s*(#[a-f][a-f0-9]{5}|#[c-f][0-9a-f]{2,5}|white|#fff)/i,
			);
