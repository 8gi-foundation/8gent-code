...
const CATEGORY_ROUNDS: CategoryRound[] = [
	{ name: "bug-fixing", tier: "EASY", benchmarks: bugFixingBenchmarks },
	{
		...
		tier: "EASY",
		benchmarks: fileManipulationBenchmarks,
	},
	{
		name: "feature-implementation",
		tier: "MEDIUM",
		benchmarks: featureImplementationBenchmarks,
	},
	...
	{ name: "ui-design", tier: "HARD", benchmarks: uiDesignBenchmarks },
	{
		name: "battle-test",
		tier: "EXPERT",
		benchmarks: [...battleTestBenchmarks, ...battleTestProBenchmarks],
	},
	{
		name: "long-horizon",
		tier: "LEGENDARY",
		benchmarks: longHorizonBenchmarks,
	},
];
...
interface CompetitorResult {
	benchmarkId: string;
	score: number;
	gradeResult: CombinedGradeResult;
	rawOutput: string;
	model: string;
	temperature: number;
	durationMs: number;
...
interface RoundResult {
	...
	category: string;
	tier: string;
	eightResults: CompetitorResult[];
	claudeResults: CompetitorResult[];
	eightAvg: number;
	claudeAvg: number;
	eightWins: number;
	claudeWins: number;
...
function shouldStop(): boolean {
	const h = getPSTHour();
	return h >= STOP_HOUR && h < STOP_HOUR + 2;
}
...
const EMPTY_GRADE: CombinedGradeResult = {
	score: 0,
	execution: null,
	keyword: { score: 0, matchedKeywords: [], missedKeywords: [] },
	method: "keyword-only",
};
...
async function runEight(
	...
	for (const temp of EIGHT_TEMPERATURES) {
		try {
			...
			log(
				`    [8gent] temp=${temp} score=${result.score} (exec=${result.execution?.score ?? "-"} kw=${result.keyword.score}) ${durationMs}ms`,
			);
...
async function runClaude(
	...
	try {
		...
		log(
			`    [Claude] score=${result.score} (exec=${result.execution?.score ?? "-"} kw=${result.keyword.score}) ${durationMs}ms`,
		);
...
function formatFinalSummary(state: CompetitionState): string {
	const cats = state.rounds
		.map(
			(r) =>
				`${r.eightAvg >= r.claudeAvg ? "✅" : "❌"} ${r.category}: 8gent ${r.eightAvg} vs Claude ${r.claudeAvg}`,
		)
		.join("\n");
	const muts =
		state.mutations
...
function saveResults(state: CompetitionState): void {
	...
	const results = {
		summary: {
			...
			eightWins: state.totalEightWins,
			claudeWins: state.totalClaudeWins,
			ties: state.totalTies,
...
function generateFinalReport(state: CompetitionState): void {
	...
	for (const r of state.rounds) {
		...
		log(
			`    ${r.category.padEnd(25)} 8gent=${String(r.eightAvg).padStart(3)} Claude=${String(r.claudeAvg).padStart(3)} => ${w}`,
		);
	}
...
async function main(): Promise<void> {
	...
	log(
		`║  Categories: ${CATEGORY_ROUNDS.length} | Benchmarks: ${totalBenchmarks} | Max rounds: ${MAX_ROUNDS || "all"}`,
	);
	log(
		`║  Stop: ${STOP_HOUR}:00 PST | 8gent temps: ${EIGHT_TEMPERATURES.join(",")} | Claude temp: ${CLAUDE_TEMPERATURE}`,
	);
	log("╚══════════════════════════════════════════════════════════════╝\n");
	...
	for (let i = startRound; i < endRound; i++) {
		if (shouldStop()) {
			...
			await sendTelegram(
				`⏸ *Paused* at round ${i + 1}/${CATEGORY_ROUNDS.length}. Resume by re-running.`,
			);
			break;
		}
		...
		log(
			`  Round wins: 8gent=${roundResult.eightWins} Claude=${roundResult.claudeWins} Ties=${roundResult.ties}`,
		);
		log(
			`  Cumulative: 8gent=${state.totalEightWins} Claude=${state.totalClaudeWins} Ties=${state.totalTies}`,
		);
