...
Cross-process clipboard synchronization backed by a shared JSON file in the OS temp directory. Provides file-based locking to prevent concurrent write collisions between agent processes.
...
- Pass file paths or short snippets between TUI and sub-agents
- Inspector/debugger can read agent clipboard history for diagnostics

## Known limitations

- Spin lock blocks the event loop briefly per cycle - acceptable for short critical sections
...
2. Expose in CLUI via a clipboard panel in the debugger
3. Consider replacing spin lock with `flock` via child_process for production use
