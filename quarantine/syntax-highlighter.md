...
1. **TUI chat screen** - wrap code blocks in `highlight()` before rendering with Ink `<Text>`. Detect language from fenced code block tag.
...
`packages/tools/syntax-highlighter.ts` (~150 lines, self-contained)
