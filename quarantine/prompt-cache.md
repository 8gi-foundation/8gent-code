...
- Similarity scan is O(n) over the cache - acceptable at <=200 entries. If capacity grows above ~1000 entries, replace with an ANN index.
- No persistence by default. To persist across sessions, serialize entries to `.8gent/prompt-cache.json` on shutdown and reload on init.
