...
`packages/tools/token-estimator.ts` (~260 lines)
...
import {
  ...
  fullReport,
  MODELS,
} from './packages/tools/token-estimator.ts';
...
// Full report across all models (string)
console.log(fullReport(myText));
```

## Heuristic

Content type is auto-detected from char density of `{}()=>; ` and indentation ratio:
...
# Pipe
cat packages/eight/prompts/system-prompt.ts | bun run packages/tools/token-estimator.ts

# Single model
bun run packages/tools/token-estimator.ts --model gpt-4o --file README.md

# Custom expected output size
