...
- Does NOT touch any existing files in the repo
- Cap: reports at most 50 similar block groups and 20 duplicate file groups in human output (full data available via `--json`)
...
- Similar block detection is line-normalized (trims whitespace, strips `//` and `#` comments) but not AST-based - may produce false positives for short generic blocks
- Large repos with many files may be slow on first run (no caching yet)
...
Once reviewed, wire into `packages/validation/index.ts` and optionally call from the post-session reflection loop in `packages/self-autonomy/reflection.ts` to track duplicate accumulation over time.
