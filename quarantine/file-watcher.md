...
const watcher = new FileWatcher({
  patterns: ["*.ts", "*.json"], // default: ["*"] (all files)
...
- Uses `fs.watch` which has OS-level quirks on macOS (FSEvents) vs Linux (inotify)
...
- No chokidar dependency - keeps the tool self-contained and lightweight
