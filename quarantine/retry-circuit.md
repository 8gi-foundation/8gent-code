...
fetch.reset(); // reset all counters and close the circuit
```
...
```ts
{
  ...
  failureCount: number;      // calls that exhausted all retries
  rejectedCount: number;     // calls rejected by open circuit
  consecutiveFailures: number;
  openedAt: number | null;   // timestamp circuit was opened
}
```
