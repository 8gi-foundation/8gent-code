...
## Integration Path
1. Register in `packages/tools/index.ts` (or equivalent tool registry) once that file exists.
2. Wire into the agent code-editing tool chain - replace or augment the current string-patch approach in `packages/eight/tools.ts` for large file edits.
...
4. Consider exposing as an MCP tool: `rope_insert`, `rope_delete`, `rope_view` for agent-driven large-file surgery.
