...
Spawns and manages child processes with full output capture. Handles timeout enforcement (SIGKILL on expiry), stdout/stderr streaming via callbacks, environment inheritance with optional override, and optional shell mode for pipeline commands. Safe by default - no shell invocation unless shell:true is explicitly passed.
...
2. Use in packages/eight/tools.ts to replace any ad-hoc Bun.spawn calls.
