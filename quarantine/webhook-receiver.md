...
To expose via the daemon, add to `packages/daemon/index.ts` inside `main()`:

```ts
import { WebhookReceiver } from "../tools/webhook-receiver";
const webhooks = new WebhookReceiver({ port: 18791 });
webhooks.start();
// register routes as needed
```

## Why quarantined

- Does not modify any existing files - zero blast radius
- Integration requires wiring into daemon config and adding secrets management
...
const r = new WebhookReceiver({ port: 19000, secret: 'test-secret', enforceSignatures: false });
