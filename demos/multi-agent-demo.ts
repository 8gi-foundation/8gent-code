...
const actions = [
	createAction({
		...
		delegatable: true,
		prompt:
			"Analyze the codebase and design a REST API schema covering users, sessions, and resources. Output an OpenAPI 3.0 spec.",
	}),
	createAction({
		...
		delegatable: true,
		prompt:
			"Build JWT-based auth middleware with login, register, and token refresh endpoints. Include rate limiting.",
	}),
	createAction({
		...
		delegatable: true,
		prompt:
			"Implement CRUD endpoints for all resources defined in the API schema. Use the auth middleware for protected routes.",
	}),
	createAction({
		...
		delegatable: true,
		prompt:
			"Write integration tests for all CRUD endpoints. Cover auth flows, validation errors, and edge cases.",
	}),
	createAction({
		...
		delegatable: true,
		prompt:
			"Generate API documentation from the OpenAPI spec. Include usage examples and authentication guide.",
	}),
];
