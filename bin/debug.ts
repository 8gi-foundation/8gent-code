...
function cmdSessions(): void {
	...
	if (files.length === 0) {
		...
		console.log(`${c.dim}Sessions are created when you run the TUI.${c.reset}`);
		return;
	}
...
function cmdInspect(id: string): void {
	...
	console.log(`${c.dim}Format: ${session.format}${c.reset}`);
	console.log();
...
function cmdTail(): void {
			...
			if (isJsonl) {
				...
				for (const line of newLines) {
					try {
						...
						const ts = entry.timestamp ? shortTimestamp(entry.timestamp) : "?";
	...
	function printJsonlEvent(ts: string, entry: Record<string, unknown>): void {
		switch (entry.type) {
			case "user_message": {
				const msg = entry.message as Record<string, unknown> | undefined;
	...
	function printLegacyEvent(ts: string, event: Record<string, unknown>): void {
