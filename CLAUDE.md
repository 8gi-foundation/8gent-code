...
- **Multi-agent orchestration:** First-class. `packages/daemon/agent-pool.ts` holds up to 10 concurrent sessions. `packages/eight/agent.ts` supports sub-agent forking so a parent can spawn children that inherit full context. Sessions carry a `channel` field (`os`, `app`, `telegram`, `discord`, `api`) — extensible for third-party terminal hosts (cmux, etc.).
