...
const RULES: PackagingRule[] = [
	{
		name: "STATEMENTS_OVER_QUESTIONS",
		weight: 2,
		description: "Statements outperform questions universally",
		dataPoint: "0% of top 20% used questions",
	},
	{
		name: "CONCRETE_OVER_ABSTRACT",
		weight: 2,
		description: "Concrete specifics beat abstract language",
		dataPoint: "270x gap between $100k vs monetize",
	},
	{
		name: "PROOF_OVER_PROMISE",
		weight: 2,
		description: "Lead with proof, not promise",
		dataPoint: "66% top performers led with proof - 3.5x over curiosity",
	},
	{
		name: "PROVEN_FORMULAS",
		weight: 1,
		description: "Use one of the 9 proven formula structures",
		dataPoint: "91% of top performers used a known formula",
	},
	{
		name: "FIRST_LINE_IS_DISTRIBUTION",
		weight: 2,
		description: "First 8 words must hook",
		dataPoint: "405:1 gap from packaging alone",
	},
	{
		name: "PACKAGING_WEIGHT",
		weight: 1,
		description: "Packaging is 60% of performance",
		dataPoint: "Quality only accounts for 15%",
	},
	{
		name: "SPECIFICITY_KILLS_VAGUENESS",
		weight: 2,
		description: "Specific outcomes beat vague goals",
		dataPoint: "1,000 followers beats grow your audience",
	},
];
...
const FORMULA_DETECTORS: Array<{ name: FormulaName; pattern: RegExp }> = [
	...
	{
		name: "number-timeframe-outcome",
		pattern: /^\d+\s+(days?|months?|weeks?)\s+to\b/i,
	},
];
...
function evaluateRule(text: string, rule: PackagingRule): RuleScore {
	...
	switch (rule.name) {
		case "STATEMENTS_OVER_QUESTIONS": {
			...
			return {
				rule: rule.name,
				score: isQuestion ? -20 : 10,
				passed: !isQuestion,
				feedback: isQuestion
					? "Questions kill reach - 0% of top performers used them. Rewrite as a statement."
		...
		case "CONCRETE_OVER_ABSTRACT": {
			...
			if (hasConcrete && !hasAbstract)
				return {
					...
					score: 15,
					passed: true,
					feedback: "Strong - concrete specifics present.",
				};
			if (hasConcrete && hasAbstract)
				return {
					rule: rule.name,
			...
			if (hasAbstract)
				return {
					...
					score: -15,
					passed: false,
					feedback: `Abstract language detected. Replace with specific numbers or outcomes. Flagged: ${ABSTRACT_WORDS.filter((w) => lower.includes(w)).join(", ")}`,
				};
			return {
				rule: rule.name,
				score: 0,
		...
		case "PROOF_OVER_PROMISE": {
			...
			if (hasProof)
				return {
					...
					score: 15,
					passed: true,
					feedback: "Proof-first hook - 3.5x better than curiosity.",
				};
			if (hasPromise)
				return {
					rule: rule.name,
		...
		case "PROVEN_FORMULAS": {
			...
			if (match)
				return {
					...
					score: 10,
					passed: true,
					feedback: `Matches proven formula: ${match.name} (avg ${FORMULAS[match.name].avgPerformance.toLocaleString()} views/day).`,
				};
			return {
				rule: rule.name,
				score: 0,
		...
		case "FIRST_LINE_IS_DISTRIBUTION": {
			...
			if (hasHookElement)
				return {
					...
					score: 10,
					passed: true,
					feedback: "First 8 words contain a hook element.",
				};
			if (isGeneric)
				return {
					rule: rule.name,
		...
		case "PACKAGING_WEIGHT": {
			...
			if (hasConcrete && hasFormula)
				return {
					...
					score: 5,
					passed: true,
					feedback: "Packaging fundamentals are solid.",
				};
			return {
				rule: rule.name,
				score: 0,
		...
		case "SPECIFICITY_KILLS_VAGUENESS": {
			...
			if (hasSpecific)
				return {
					...
					score: 10,
					passed: true,
					feedback: "Specific outcome stated.",
				};
			return {
				rule: rule.name,
				score: 0,
