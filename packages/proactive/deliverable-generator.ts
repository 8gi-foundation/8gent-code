 ...
 */
export function estimateEffort(opportunity: Opportunity): {
	hours: number;
	complexity: string;
} {
	return (
		EFFORT_TABLE[opportunity.estimatedEffort] ?? {
			hours: 4,
			complexity: "unknown",
		}
	);
...
export function isActionable(deliverable: Deliverable, maxHours = 8): boolean {
	const effort = Object.values(EFFORT_TABLE).find((e) =>
		deliverable.estimatedTime.includes(String(e.hours)),
	);
