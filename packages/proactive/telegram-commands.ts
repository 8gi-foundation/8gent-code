...
async function healthCommand(_args: string): Promise<CommandResult> {
  ...
  const typeErrors = tscOutput
    ? (tscOutput.match(/error TS/g) || []).length
    : -1;
  ...
  if (typeErrors < 0) score -= 10; // could not run tsc
...
async function standupCommand(_args: string): Promise<CommandResult> {
  ...
  if (commitLines.length === 0) {
    ...
    if (commitLines.length > 8) {
      lines.push(`  ... and ${commitLines.length - 8} more`);
    }
  }
...
async function scanCommand(args: string): Promise<CommandResult> {
  // Scan GitHub for bounty/opportunity issues
  ...
  try {
    ...
    for (const issue of issues) {
      ...
      const age = timeSince(new Date(issue.updatedAt));
