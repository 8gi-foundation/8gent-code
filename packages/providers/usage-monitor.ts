...
export class UsageMonitor {
	...
	check(): {
		...
		const weeklyTokens = this.state.records
			.filter((r) => r.date >= ws)
		...
		if (dailyTokens >= b.dailyTokens) {
			return {
				allowed: false,
				reason: `Daily token limit reached (${dailyTokens.toLocaleString()}/${b.dailyTokens.toLocaleString()})`,
		...
		if (weeklyTokens >= b.weeklyTokens) {
			return {
				allowed: false,
				reason: `Weekly token limit reached (${weeklyTokens.toLocaleString()}/${b.weeklyTokens.toLocaleString()})`,
				dailyUsed: dailyTokens,
				weeklyUsed: weeklyTokens,
				dailyPct,
				weeklyPct,
		...
		if (dailyReqs >= b.dailyRequests) {
			return {
				allowed: false,
				reason: `Daily request limit reached (${dailyReqs}/${b.dailyRequests})`,
				dailyUsed: dailyTokens,
				weeklyUsed: weeklyTokens,
				dailyPct,
				weeklyPct,
