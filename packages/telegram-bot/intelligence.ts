...
export class GitHubIntelligence {
	...
	async runNightlyIntelligence(): Promise<string> {
		...
		const searches = [
			{
				query: "stars:>50 pushed:>2026-03-01",
				sort: "stars" as const,
				label: "overall",
			},
			{
				query: "language:typescript stars:>20 pushed:>2026-03-01",
				sort: "stars" as const,
				label: "typescript",
			},
			{
			...
			{
				query: "MCP OR model-context-protocol stars:>5 pushed:>2026-03-01",
				sort: "updated" as const,
				label: "mcp",
			},
		];
		...
		console.log(
			`[intel] Done. ${relevant} relevant, ${newCount} new. ${this.knowledgeBase.length} total tracked.`,
		);
	...
	formatDigest(digest: IntelligenceDigest): string {
		...
		lines.push("");
		lines.push(
			`Scanned: ${digest.scanned} repos | Relevant: ${digest.relevant} | New: ${digest.newDiscoveries}`,
		);
		lines.push("");
