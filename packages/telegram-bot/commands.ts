...
async function handleMutations(
	...
	await bot.sendMessage(formatMutationList(mutations), {
		parseMode: "Markdown",
	});
}
...
async function handleStart(
	...
	try {
		...
		if (!existsSync(scriptPath)) {
			await bot.sendMessage("❌ Script not found: `scripts/nightly-train.ts`", {
				parseMode: "Markdown",
			});
			return;
		}
		...
		await bot.sendMessage(`❌ *Failed to start:* ${err.message}`, {
			parseMode: "Markdown",
		});
	}
}
...
async function handleStop(
	...
	try {
		...
		await bot.sendMessage(`❌ *Failed to stop:* ${err.message}`, {
			parseMode: "Markdown",
		});
	}
}
...
async function handleTrending(
	...
	try {
		...
		await bot.sendMessage(`*Trending scan failed:* ${err.message}`, {
			parseMode: "Markdown",
		});
	}
}
...
export const commands: CommandDefinition[] = [
	...
	{
		name: "compare",
		description: "Show 8gent vs Claude comparison",
		handler: handleCompare,
	},
	{
		name: "round",
		description: "Show details of competition round N",
		handler: handleRound,
	},
	{
		name: "mutations",
		description: "Show accumulated mutations",
	...
	{
		name: "start",
		description: "Start overnight competition run",
		handler: handleStart,
	},
	{
		name: "stop",
		description: "Stop current run gracefully",
		handler: handleStop,
	},
	{
		name: "intel",
		description: "Show latest GitHub intelligence digest",
		handler: handleIntel,
	},
	{
		name: "repos",
		description: "List tracked repos by relevance score",
		handler: handleRepos,
	},
	{
		name: "trending",
		description: "Run ad-hoc GitHub trending scan",
