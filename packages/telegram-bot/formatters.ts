...
export function formatCompetitionRound(round: CompetitionRound): string {
	...
	if (round.tokens) {
		header.push(
			`🔤 Tokens: ${formatTokens(round.tokens.total)} (${formatTokens(round.tokens.input)}↓ ${formatTokens(round.tokens.output)}↑)`,
		);
	}
...
export function formatBenchmarkReport(report: BenchmarkReport): string {
	...
	// Footer
	sections.push(
		`_${new Date(report.timestamp).toLocaleString("en-US", { timeZone: "America/Los_Angeles" })} PST_`,
	);
...
export function formatMorningBrief(summary: OvernightSummary): string {
	...
	sections.push("");
	sections.push(
		`🏆 *Best Round:* #${summary.bestRound.round} (${summary.bestRound.score.toFixed(1)})`,
	);
	sections.push(
		`💀 *Worst Round:* #${summary.worstRound.round} (${summary.worstRound.score.toFixed(1)})`,
	);
	...
	sections.push(`${horizontalRule()}`);
	sections.push(
		`_${new Date(summary.startTime).toLocaleDateString("en-US", { timeZone: "America/Los_Angeles", weekday: "long", month: "long", day: "numeric" })}_`,
	);
	sections.push(
		`_Run: ${new Date(summary.startTime).toLocaleTimeString("en-US", { timeZone: "America/Los_Angeles", hour: "2-digit", minute: "2-digit" })} → ${new Date(summary.endTime).toLocaleTimeString("en-US", { timeZone: "America/Los_Angeles", hour: "2-digit", minute: "2-digit" })} PST_`,
	);
