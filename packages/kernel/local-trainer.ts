...
export class LocalTrainer {
	...
	async runTraining(): Promise<CheckpointInfo> {
		...
		try {
			...
			console.log(
				`[local-trainer] Starting training: ${consumedCount} pairs, rank=${this.config.loraRank}`,
			);
			...
			if (benchScore >= this.config.promotionThreshold) {
				...
				console.log(
					`[local-trainer] Checkpoint below threshold (${benchScore} < ${this.config.promotionThreshold}), rolling back`,
				);
	...
	private async spawnTrainer(
		...
		const args = [
			...
			dataPath,
			"--base-model",
			this.config.baseModelHf,
			"--output",
			outputDir,
			"--epochs",
			String(this.config.epochs),
			"--lora-rank",
			String(this.config.loraRank),
			"--lr",
			String(this.config.learningRate),
			"--batch-size",
