...
export async function judgeImprovement(
	...
	if (delta <= 0) {
		return {
			verdict: "none",
			reasoning: `No improvement (delta: ${delta.toFixed(1)})`,
		};
	}
	if (delta < 2) {
		return {
			verdict: "patch",
			reasoning: `Marginal improvement (+${delta.toFixed(1)}), patch only`,
		};
	}
...
OLD SCORES (baseline):
${Object.entries(oldDetails)
	.map(([k, v]) => `  ${k}: ${v}`)
	.join("\n")}
Average: ${oldScore.toFixed(1)}

NEW SCORES (candidate):
${Object.entries(newDetails)
	.map(([k, v]) => `  ${k}: ${v}`)
	.join("\n")}
