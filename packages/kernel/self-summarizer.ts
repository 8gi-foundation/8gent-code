...
export class SelfSummarizer {
  ...
  async summarize(messages: Message[]): Promise<SummaryResult> {
    ...
    const retentionScore =
      pathScore * 0.5 + errorScore * 0.3 + Math.min(decisionsKept / 5, 1) * 0.2;
...
COMPRESSED SUMMARY:`;
  ...
  private async callOllama(prompt: string): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(
        `Ollama summarization failed: ${response.status} ${response.statusText}`
      );
    }
  ...
  private extractFilePaths(text: string): string[] {
    ...
    const pathRegex =
      /(?:\/[\w./-]+|(?:src|packages|apps|lib|config|docs)\/[\w./-]+)/g;
