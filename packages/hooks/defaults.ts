...
export const commandLoggingHook: Omit<Hook, "id"> = {
	...
	mode: "shell",
	command:
		'echo "[$(date -Iseconds)] Command: {command} (exit: {exitCode})" >> ~/.8gent/logs/commands.log',
	enabled: false,
	continueOnError: true,
	async: true,
};
...
export const telegramNotificationHook: Omit<Hook, "id"> = {
	...
	description:
		"Sends Telegram message on completion (requires TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID env vars)",
	mode: "shell",
	command:
		'curl -s -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage" -d "chat_id=$TELEGRAM_CHAT_ID&text=8gent: Task completed"',
	enabled: false,
	continueOnError: true,
	async: true,
};
...
export const autoFormatHook: Omit<Hook, "id"> = {
	...
	mode: "shell",
	command:
		'[ "{tool}" = "write_file" ] && bunx prettier --write "$(dirname {workingDirectory})/$(basename {toolInput.path})" 2>/dev/null || true',
	enabled: false,
	continueOnError: true,
	async: true,
};
