...
function buildControlPanel(agent: any): { text: string; keyboard: any } {
	...
	const uptime = agent
		? Math.round((Date.now() - (agent.sessionStartTime ?? Date.now())) / 60000)
		: 0;
			...
			[
				{
					...
					callback_data: "model:anthropic/claude-sonnet-4",
				},
				{
					text: "Claude Opus 4",
					callback_data: "model:anthropic/claude-opus-4",
				},
			],
			[
				...
				{
					text: "Gemini 2.5 Pro",
					callback_data: "model:google/gemini-2.5-pro",
				},
			],
...
export class TelegramBot {
	...
	private async handleCallback(query: CallbackQuery): Promise<void> {
		...
		if (data === "panel:main" || data === "panel:refresh") {
			...
			await answerCallback(
				this.config.token,
				id,
				data === "panel:refresh" ? "Refreshed" : undefined,
			);
			return;
		}
	...
	private async handleMessage(update: TelegramUpdate): Promise<void> {
		...
		if (text.startsWith("/btw ")) {
			...
			// Typing indicator
			api(this.config.token, "sendChatAction", {
				chat_id: chat.id,
				action: "typing",
			}).catch(() => {});
