...
registerTool(
	{
		name: "get_symbol",
		description:
			"Get the source code for a specific symbol by its ID. Much more efficient than reading the full file.",
	...
	async (input: unknown, context: ExecutionContext): Promise<SymbolOutput> => {
		...
		if (separatorIndex === -1) {
			throw new Error(
				`Invalid symbol ID format. Expected 'path/to/file.ts::symbolName', got '${symbolId}'`,
			);
		}
		...
		const symbol = outline.symbols.find(
			(s) => s.name === symbolName || s.id.endsWith(`::${symbolName}`),
		);
