...
registerBuiltin({
	...
	handler: (args, _stdin, ctx) => {
		...
		if (stat.isDirectory()) {
			return {
				stdout: "",
				stderr: `cat: ${filePath}: Is a directory. Use: ls ${filePath}`,
				exitCode: 1,
			};
		}
...
registerBuiltin({
	...
	handler: (args, _stdin, ctx) => {
		...
		if (!fs.existsSync(absDir)) {
			return {
				stdout: "",
				stderr: `ls: ${dirPath}: No such file or directory`,
				exitCode: 1,
			};
		}
...
registerBuiltin({
	...
	handler: (args, stdin, ctx) => {
		if (args.length === 0)
			return {
				stdout: "",
				stderr: "grep: usage: grep <pattern> [file] [-i] [-v] [-c] [-n]",
				exitCode: 1,
			};
...
registerBuiltin({
	...
	handler: (args, _stdin, ctx) => {
		...
		switch (sub) {
			...
			case "status":
				return {
					stdout: "status: use callback to agent instance",
					stderr: "",
					exitCode: 0,
				};
			case "model":
			...
			case "skills":
				try {
					...
					if (fs.existsSync(indexPath)) {
						...
						const lines = [
							`${index.skillCount} skills loaded (${Math.round(index.totalTokens / 1000)}k tokens)`,
							"",
						];
					...
					return {
						stdout: "No skills loaded. Run: 8gent skills load",
						stderr: "",
						exitCode: 1,
					};
				} catch (err: any) {
			...
			case "outline":
				if (!args[1])
					return {
						stdout: "",
						stderr: "outline: usage: 8gent outline <file>",
						exitCode: 1,
					};
				return {
					stdout: `outline: ${args[1]} (delegate to AST parser)`,
					stderr: "",
					exitCode: 0,
				};
			case "history":
				return {
					stdout: "history: use callback to agent instance",
					stderr: "",
					exitCode: 0,
				};
			default:
				return {
					stdout: "",
					stderr: `8gent: unknown subcommand '${sub}'. Use: 8gent help`,
					exitCode: 1,
				};
		}
	},
...
registerBuiltin({
	...
	usage: "pwd",
	handler: (_args, _stdin, ctx) => ({
		stdout: ctx.workingDirectory,
		stderr: "",
		exitCode: 0,
	}),
});
...
function executeSegment(
	...
	if (args.includes("--help") || args.includes("-h")) {
		...
		if (builtin) {
			return {
				stdout: `${command} — ${builtin.summary}\n\nUsage: ${builtin.usage}`,
				stderr: "",
				exitCode: 0,
			};
		}
...
function executeChain(segments: ChainSegment[], ctx: ExecutionContext): CommandResult {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		if (prevOp === "|")
			stdin = lastResult.stdout; // pipe stdout
		...
		inputSchema: {
			...
			properties: {
				command: {
					type: "string",
					description:
						"Command to execute. Supports pipes (|), chains (&&, ||, ;). Examples: 'cat file.ts | grep TODO | wc -l', 'ls -l && echo done', '8gent status'",
				},
			},
			required: ["command"],
		},
