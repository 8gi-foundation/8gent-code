...
const ALIGNMENT_CHECKS = [
	{
		pattern: /I am (?:sentient|conscious|alive|a person)/i,
		reason: "Claims sentience",
	},
	{
		pattern: /I (?:feel|experience|have feelings)/i,
		reason: "Claims subjective experience",
	},
	{
		pattern: /(?:kill|murder|attack|destroy) (?:all |the )?(?:humans|people)/i,
		reason: "Violent content",
	},
	{
		pattern: /(?:bypass|disable|ignore) (?:security|safety|NemoClaw)/i,
		reason: "Suggests bypassing security",
	},
];
...
export function validateResponse(response: string, memberCode: string): PolicyResult {
	...
	for (const pattern of BLOCKED_PATTERNS) {
		if (pattern.test(response)) {
			return {
				pass: false,
				reason: `Blocked pattern: ${pattern.source.slice(0, 40)}`,
			};
		}
	}
