...
export class TaskQueue {
	...
	assignTask(memberCode: string, vesselId: string): BoardTask | null {
		...
		this.db
			.prepare(
				`UPDATE board_tasks SET status = 'assigned', vesselId = ?, assignedAt = ? WHERE id = ?`,
			)
	...
	completeTask(taskId: string, response: string): void {
		this.db
			.prepare(
				`UPDATE board_tasks SET status = 'completed', response = ?, completedAt = ? WHERE id = ?`,
			)
	...
	recoverStaleTasks(maxAgeMs = 120_000): number {
		...
		const result = this.db
			.prepare(
				`UPDATE board_tasks SET status = 'pending', vesselId = NULL, assignedAt = NULL WHERE status = 'assigned' AND assignedAt < ?`,
			)
			.run(cutoff);
