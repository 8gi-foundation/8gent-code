...
const SAFE_TOOLS = new Set([
  "get_outline", "get_symbol", "search_symbols", "get_project_outline",
  "read_file", "list_files", "git_status", "git_diff", "git_log",
  "read_pdf", "read_pdf_page", "search_pdf",
  ...
  private async handleToolsCall(req: JSONRPCRequest): Promise<JSONRPCResponse> {
    ...
    if (this.toolFilter === "safe" && !SAFE_TOOLS.has(params.name)) {
      return {
        jsonrpc: "2.0",
        id: req.id!,
        error: { code: -32602, message: `Tool "${params.name}" not available in safe mode` },
      };
    }
