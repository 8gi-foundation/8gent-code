...
export class ConvexTenantStore implements TenantStore {
	...
	async getById(tenantId: string): Promise<TenantConfig | null> {
		const doc = (await this.client.query("tenants:get", {
			tenantId,
		})) as ConvexTenantDoc | null;
		return doc ? docToConfig(doc) : null;
	}

	async getBySubdomain(subdomain: string): Promise<TenantConfig | null> {
		const doc = (await this.client.query("tenants:getBySubdomain", {
			subdomain,
		})) as ConvexTenantDoc | null;
		return doc ? docToConfig(doc) : null;
	}
	...
	async delete(tenantId: string): Promise<boolean> {
		return (await this.client.mutation("tenants:deleteTenant", {
			tenantId,
		})) as boolean;
	}
}
