...
export class SoxSynth {
	...
	private kick(path: string): void {
		...
		execSync(
			`${SOX} -n -r ${SAMPLE_RATE} ${path} synth 0.3 sine 150:40 vol 0.9 fade l 0.005 0.3 0.1`,
		);
	}
	...
	private hihat(path: string): void {
		execSync(
			`${SOX} -n -r ${SAMPLE_RATE} ${path} synth 0.05 noise vol 0.25 highpass 8000 fade l 0.001 0.05 0.03`,
		);
	}
	...
	private openHihat(path: string): void {
		execSync(
			`${SOX} -n -r ${SAMPLE_RATE} ${path} synth 0.2 noise vol 0.2 highpass 6000 fade l 0.001 0.2 0.15`,
		);
	}
	...
	private bassNote(path: string, freq: number, duration: number): void {
		execSync(
			`${SOX} -n -r ${SAMPLE_RATE} ${path} synth ${duration} sine ${freq} vol 0.6 fade l 0.01 ${duration} 0.05 lowpass 200`,
		);
	}
	...
	private padChord(path: string, freqs: number[], duration: number): void {
		const parts = freqs.map((f, i) => {
			...
			execSync(
				`${SOX} -n -r ${SAMPLE_RATE} ${part} synth ${duration} sine ${f} vol ${0.15 / freqs.length} fade l 0.5 ${duration} 1.0`,
			);
			return part;
		});
		execSync(`${SOX} -m ${parts.join(" ")} ${path}`);
		try {
			execSync(`rm ${parts.join(" ")}`);
		} catch {}
	}
	...
	private melodyNote(path: string, freq: number, duration: number): void {
		execSync(
			`${SOX} -n -r ${SAMPLE_RATE} ${path} synth ${duration} square ${freq} vol 0.2 fade l 0.01 ${duration} 0.05 lowpass 3000`,
		);
	}
	...
	generateDrums(
		bpm: number,
		bars: number,
		pattern: "four-four" | "breakbeat" | "dnb" = "four-four",
	): string {
		...
		// Clean up temp files
		try {
			execSync(`rm ${kickPath} ${snarePath} ${hihatPath} ${ohPath}`);
		} catch {}
	...
	private mixAt(trackPath: string, samplePath: string, offsetSec: number): void {
		...
		try {
			execSync(`rm ${padded} ${tmpPath} 2>/dev/null`);
		} catch {}
	}
