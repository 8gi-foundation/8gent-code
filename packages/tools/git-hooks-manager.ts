...
export class GitHooks {
  ...
  /** Remove an installed hook. Returns false if not present. */
  ...
  /** List all known hook types with installed status and content. */
  list(): HookInfo[] {
    const all: HookType[] = [
      "pre-commit",
      "pre-push",
  ...
  /** Run a hook directly (bypassing git). Returns exit code + stdout/stderr. */
   ...
   * Scripts run in order with set -e - any failure stops the chain.
   */
