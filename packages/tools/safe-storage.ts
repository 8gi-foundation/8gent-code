 ...
 * Storage: Single encrypted JSON file on disk. Each value gets its own IV + auth tag.
 */
...
export class SafeStorage {
  ...
  /**
   * @param filePath - Path to the encrypted storage file (created if absent).
   * @param passphrase - Passphrase for key derivation. Must be at least 8 chars.
   */
  ...
  private load(): StorageFile {
    ...
    try {
      ...
      throw new Error(`SafeStorage: failed to read storage file at ${this.filePath}`);
    }
  }
  ...
  get(key: string): string | null {
    ...
    try {
      ...
      throw new Error(`SafeStorage: decryption failed for key "${key}" - wrong passphrase or corrupted data`);
    }
  }
