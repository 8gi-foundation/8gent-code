...
export class Counter<T> {
  ...
  constructor(initial?: Iterable<T> | Map<T, number>) {
    this.counts = new Map();
  ...
  /** Remove a key entirely regardless of count. */
  ...
  /** All [key, count] pairs sorted descending by count. */
  entries(): [T, number][] {
  ...
  /**
   * Subtract counts from another Counter in-place.
   * Removes keys whose count reaches or goes below zero. Returns this.
   */
  ...
  /** Returns a plain object snapshot (keys coerced to string). */
  ...
  /** Returns a new Counter that is an independent copy of this one. */
  clone(): Counter<T> {
