 ...
 *   streamEncoder(onChunk, chunkSize) - streaming encode for large data
 */
...
// ---------------------------------------------------------------------------
// URL-safe base64  (RFC 4648 §5 — replaces +/= with -_)
// ---------------------------------------------------------------------------
...
export function decodeUrlSafe(input: string): string {
  ...
  // Re-add padding and convert back to standard base64 chars before decoding.
...
/**
 * Return true if the input is a valid URL-safe base64 string (no padding).
 */
 ...
 *
 * chunkSize MUST be a multiple of 3 to avoid mid-stream padding issues.
 */
