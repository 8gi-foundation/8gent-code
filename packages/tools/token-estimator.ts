...
export const MODELS: ModelSpec[] = [
  { id: "claude-3-5-sonnet", label: "Claude 3.5 Sonnet", contextWindow: 200_000, inputCostPer1k: 0.003, outputCostPer1k: 0.015 },
  { id: "claude-3-haiku", label: "Claude 3 Haiku", contextWindow: 200_000, inputCostPer1k: 0.00025, outputCostPer1k: 0.00125 },
  { id: "claude-opus-4", label: "Claude Opus 4", contextWindow: 200_000, inputCostPer1k: 0.015, outputCostPer1k: 0.075 },
  { id: "gpt-4o", label: "GPT-4o", contextWindow: 128_000, inputCostPer1k: 0.0025, outputCostPer1k: 0.01 },
  { id: "gpt-4o-mini", label: "GPT-4o Mini", contextWindow: 128_000, inputCostPer1k: 0.00015, outputCostPer1k: 0.0006 },
  { id: "o1", label: "OpenAI o1", contextWindow: 200_000, inputCostPer1k: 0.015, outputCostPer1k: 0.06 },
  { id: "gemini-2-flash", label: "Gemini 2.0 Flash", contextWindow: 1_048_576, inputCostPer1k: 0.0001, outputCostPer1k: 0.0004 },
  { id: "gemini-2-pro", label: "Gemini 2.0 Pro", contextWindow: 2_000_000, inputCostPer1k: 0.0035, outputCostPer1k: 0.014 },
  { id: "llama-3-70b", label: "Llama 3 70B (OpenRouter free)", contextWindow: 128_000, inputCostPer1k: 0, outputCostPer1k: 0 },
  { id: "qwen-3.5-local", label: "Qwen 3.5 (Ollama local)", contextWindow: 32_768, inputCostPer1k: 0, outputCostPer1k: 0 },
];
...
export function estimateCost(model: ModelSpec, inputTokens: number, outputTokens = 512): CostEstimate {
  const inputCostUsd = (inputTokens / 1000) * model.inputCostPer1k;
  const outputCostUsd = (outputTokens / 1000) * model.outputCostPer1k;
  return { model, inputTokens, outputTokens, inputCostUsd, outputCostUsd, totalCostUsd: inputCostUsd + outputCostUsd };
}
...
function fmt(n: number, decimals = 6): string {
  return n === 0 ? "$0.000000" : `$${n.toFixed(decimals)}`;
}
...
export function formatEstimate(estimate: TokenEstimate): string {
  return [
    ...
    `Tokens: ${thousands(estimate.tokens)}  (${estimate.method} heuristic, ~${CHARS_PER_TOKEN[estimate.method]} chars/token)`,
  ].join("\n");
}
...
export function fullReport(text: string, outputTokens = 512): string {
  ...
  for (const model of MODELS) {
    ...
    sections.push(`Context: ${ctx.usedPercent}% of ${thousands(ctx.contextWindow)}${ctx.overLimit ? "  [OVER LIMIT]" : ""}`);
...
`);
  ...
  if (args.includes("--list-models")) {
    ...
    for (const m of MODELS) {
      ...
      console.log(`  ${m.id.padEnd(22)} ${m.label.padEnd(30)} ctx: ${thousands(m.contextWindow)}${free}`);
    }
    process.exit(0);
  }
