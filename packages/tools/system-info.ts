...
async function collectDisk(): Promise<DiskEntry[]> {
  // -P for POSIX-compliant output across macOS and Linux
  const raw = await shell("df -P 2>/dev/null | awk 'NR>1 {print}'");
  ...
  for (const line of lines) {
    ...
    // df -P reports sizes in 512-byte blocks
    const blocks = parseInt(parts[1], 10) || 0;
    const used = parseInt(parts[2], 10) || 0;
    const avail = parseInt(parts[3], 10) || 0;
    const GB = 1024 * 1024 * 2; // 512-byte blocks per GB
    result.push({
      filesystem: parts[0],
      sizeGB: round1(blocks / GB),
      usedGB: round1(used / GB),
      availGB: round1(avail / GB),
      mountpoint,
    });
  }
  return result;
