...
function union<A, B, C>(a: Predicate<A>, b: Predicate<B>, c: Predicate<C>): Predicate<A | B | C>;
function union(...predicates: Predicate<unknown>[]): Predicate<unknown> {
	return (value: unknown): value is unknown => predicates.some((p) => p(value));
}
...
function tuple<T extends Predicate<unknown>[]>(...predicates: T): Predicate<InferTuple<T>> {
...
function literal<T extends string | number | boolean>(expected: T): Predicate<T> {
