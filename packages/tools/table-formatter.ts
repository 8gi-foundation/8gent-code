...
function buildRow(cells: string[], widths: number[], b: Borders): string {
  if (b.vert === "  ") return cells.join("  ");
  const inner = cells.map(c => ` ${c} `).join(b.vert);
  return `${b.vert}${inner}${b.vert}`;
}

function buildSep(widths: number[], b: Borders, which: "top" | "mid" | "bot"): string {
  const map = {
    top: [b.topLeft, b.top, b.topMid, b.topRight],
    mid: [b.midLeft, b.mid, b.midMid, b.midRight],
    bot: [b.botLeft, b.bot, b.botMid, b.botRight],
  };
 ...
 * @param options Configuration for columns, border style, alignment, and width.
