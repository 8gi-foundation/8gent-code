...
function detectCI(): CIProfile {
  ...
  return { isCI: !!(env.CI || env.CONTINUOUS_INTEGRATION || provider), provider, buildId, branch, commit };
}
function detectContainer(): ContainerProfile {
  ...
  try { const { existsSync } = require("fs"); isDocker = existsSync("/.dockerenv"); } catch {}
  ...
  return { isDocker, isKubernetes, isContainer: isDocker || isKubernetes || !!env.container, containerRuntime };
}
function detectOS(): OSProfile {
  return { platform: process.platform, arch: process.arch, release: process.version ?? null, hostname: process.env.HOSTNAME ?? process.env.COMPUTERNAME ?? null };
}
function detectShell(): ShellProfile {
  const env = process.env;
  const shellPath = env.SHELL ?? env.ComSpec ?? null;
  let shell: string | null = null;
  if (shellPath) { const parts = shellPath.split("/"); shell = parts[parts.length - 1] ?? null; }
  else if (process.platform === "win32") { shell = "cmd"; }
  return { shell, shellVersion: env[`${shell?.toUpperCase()}_VERSION`] ?? null, path: shellPath };
}
function detectTerminal(): TerminalProfile {
  ...
  return { term, termProgram, colorDepth, hasColor: colorDepth > 1, columns: process.stdout.columns ?? null, rows: process.stdout.rows ?? null };
}
export function detectEnv(): EnvProfile {
  return { ci: detectCI(), container: detectContainer(), os: detectOS(), shell: detectShell(), terminal: detectTerminal(), tty: process.stdout.isTTY === true };
}
