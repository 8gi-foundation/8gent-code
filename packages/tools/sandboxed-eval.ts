 ...
 * sandboxed-eval.ts
 *
 * Safely evaluates JavaScript expressions in a restricted context.
 * No dependencies. No network. No fs. No process access.
 *
 * Usage:
 *   import { safeEval } from './sandboxed-eval.ts';
 *   const result = await safeEval('1 + 2 * 3');
 *   const result = await safeEval('x * 2', { x: 5 });
 *   const result = await safeEval('slow()', {}, 500);
 */
...
// Globals that must be explicitly blocked even if they appear in the outer scope.
const BLOCKED_NAMES = [
...
/**
 * Builds the restricted scope parameter list and argument list for the
 * Function constructor wrapper. Each blocked name is shadowed to undefined.
 */
 ...
 * @returns        EvalResult with ok, value, error, and durationMs.
 */
...
if (import.meta.main) {
  ...
  if (!code) {
    console.error('Usage: bun packages/tools/sandboxed-eval.ts "<expression>"');
    process.exit(1);
  }
