 ...
 * - Zero external dependencies (Bun.serve + Web Crypto)
 */
...
export interface RouteConfig {
  path: string;
  /** HMAC-SHA256 secret. Overrides global secret for this path. */
  secret?: string;
  /** Header carrying the signature, e.g. "x-hub-signature-256". Default: "x-webhook-signature". */
...
export interface WebhookReceiverOptions {
  ...
  maxQueueSize?: number;
  /** If true, requests with invalid or missing signatures are rejected with 403. Default: true. */
...
export class WebhookReceiver {
  ...
  /** Register a route handler. Call before start(). */
  on(config: RouteConfig): this {
    this.routes.set(config.path, config);
    return this;
  }

  /** Start the HTTP server. Returns a promise that resolves once listening. */
  start(): this {
    const self = this;
