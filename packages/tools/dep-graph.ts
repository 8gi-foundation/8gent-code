...
export function buildGraph(root: string = process.cwd()): DepGraph {
  ...
  for (const file of files) {
    ...
    if (!node) continue;
    if (!isInternalDep(node.name) && node.name !== "@podjamz/8gent-code") {
      // Include any named package that lives inside the monorepo
      if (!node.name.startsWith("8gent-") && !node.name.startsWith("@8gent/") && !node.name.startsWith("@podjamz/")) {
        // Still include it if it has internal deps - it's part of the workspace
        if (node.deps.length === 0 && !file.startsWith(root + "/apps/") && !file.startsWith(root + "/packages/")) {
          continue;
        }
      }
    }
...
export function detectCycles(graph: DepGraph): Cycle[] {
  ...
  function dfs(node: string) {
    if (inStack.has(node)) {
      ...
      // Deduplicate: normalise by starting from the lexicographically smallest node
...
export function toDot(graph: DepGraph, cycles: Cycle[] = []): string {
  ...
  for (const name of graph.nodes.keys()) {
    if (name.startsWith("@8gent/") && graph.nodes.get(name)!.path.includes("/apps/")) {
      apps.push(name);
    } else if (name.startsWith("@8gent/") || name.startsWith("@podjamz/")) {
      pkgs.push(name);
    } else {
      other.push(name);
    }
