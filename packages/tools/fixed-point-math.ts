...
export class Decimal {
  ...
  add(other: Decimal): Decimal {
    ...
    return this.from(this.scaled + other.scaled);
  }
  ...
  mul(other: Decimal): Decimal {
    this.assertSamePrecision(other);
    return this.from((this.scaled * other.scaled) / this.factor);
  }
  ...
  abs(): Decimal {
    return this.from(this.scaled < 0n ? -this.scaled : this.scaled);
  }
  ...
  toNumber(): number {
    return Number(this.scaled) / Number(this.factor);
  }

  private assertSamePrecision(other: Decimal): void {
    if (this.precision !== other.precision) {
      throw new Error('Precision mismatch: ' + this.precision + ' vs ' + other.precision + '. Construct both Decimals with the same precision.');
    }
  }
}
