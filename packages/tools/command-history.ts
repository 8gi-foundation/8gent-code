  ...
  frequency(topN = 10): FrequencyEntry[] {
    ...
    return [...map.entries()]
      .map(([command, data]) => ({
        ...
        avgExitCode: data.exitCodes.reduce((a, b) => a + b, 0) / data.exitCodes.length,
      }))
