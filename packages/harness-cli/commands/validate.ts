...
function collectArtifactRefs(
	...
	for (const p of [
		...
		...(sessionEnd?.summary?.filesModified ?? []),
	]) {
...
export async function validate(args: string[]): Promise<void> {
	...
	if (!opts.sessionId) {
		console.error(
			"Usage: harness-cli validate <session-id> [--expect <substr>] [--expect-file <path>] [--strict-artifacts]",
		);
		process.exit(1);
	}
	...
	result.checks.push({
		...
		pass: !!sessionEnd,
		message: sessionEnd
			? `Completed (exit: ${sessionEnd.summary?.exitReason})`
			: "Session did not complete (no session_end entry)",
	});
	...
	if (opts.json) {
		...
		if (result.artifactChecks) {
			console.log(
				`  Artifacts checked: ${result.artifactChecks.checked.length}, missing: ${result.artifactChecks.missing.length}`,
			);
		}
