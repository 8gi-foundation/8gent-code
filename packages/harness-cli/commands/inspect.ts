...
function formatEntry(entry: SessionEntry, index: number): string {
	...
	switch (type) {
		...
		case "step_end": {
			...
			const usage = (entry as any).usage;
			lines.push(
				`${header} Step #${step} → ${reason}${usage ? ` [${usage.totalTokens} tok]` : ""}`,
			);
			break;
		}
		case "error": {
		...
		default: {
			...
			const keys = Object.keys(entry).filter(
				(k) => !["type", "timestamp", "sequenceNumber"].includes(k),
			);
			if (keys.length > 0) {
...
function printSummary(entries: SessionEntry[], sessionFilePath: string): void {
	...
	console.log(
		`  Last response: ${summary.lastResponse.slice(0, 150) || "—"}${summary.lastResponse.length > 150 ? "..." : ""}`,
	);
