...
export const EASING = {
	...
	scroll: {
		description:
			"For scroll-driven animations, use linear mapping from scroll position to property value. No easing curve needed.",
...
export const PERFORMANCE = {
	...
	webgl: {
		...
		avoid:
			"Do not use WebGL for text, buttons, cards, or anything CSS can handle. WebGL has a high initialization cost and kills mobile performance if misused.",
	},

	wasm: {
		when: "Use WASM for: image processing, audio analysis, physics simulations, or heavy computation that blocks the main thread.",
		avoid:
			"Do not use WASM for DOM manipulation, animation, or anything the browser already does well.",
	},
};
