...
function sendEvent(ws: ComputerWS, ev: StreamEvent): void {
  try {
    ws.send(envelope({ protocol_version: PROTOCOL_VERSION, type: "event", event: ev }));
  } catch {
...
function sendError(ws: ComputerWS, message: string): void {
  try {
    ws.send(envelope({ protocol_version: PROTOCOL_VERSION, type: "error", payload: { message } }));
  } catch {}
}
...
function resetIdleTimer(ws: ComputerWS, state: ComputerRouteState): void {
  ...
  const t = setTimeout(() => {
    if (state.sessionId) {
      sendEvent(ws, { kind: "done", sessionId: state.sessionId, reason: "idle-timeout" });
    }
  ...
  for (const ev of events) {
    const id = bus.on(ev, (payload: any) => {
      ...
      switch (ev) {
        ...
        case "agent:error": {
          sendEvent(ws, { kind: "error", sessionId, error: String(payload.error ?? "unknown error") });
          break;
        }
        ...
        case "session:end": {
          const reason = payload.reason as StreamEvent extends { kind: "done"; reason: infer R } ? R : never;
...
export function handleComputerOpen(
  ...
  setTimeout(() => {
    try {
      ...
      sendError(ws, `failed to create session: ${err instanceof Error ? err.message : String(err)}`);
    }
  }, 0);
  ...
  switch (msg.type) {
    ...
    case "session:destroy": {
      if (state.sessionId) {
        ...
        bus.emit("session:end", { sessionId: state.sessionId, reason: "client-destroy" });
