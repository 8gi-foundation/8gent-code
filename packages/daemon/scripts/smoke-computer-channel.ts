...
class MockAgentPool {
  ...
  createSession(sessionId: string, channel: string): void {
    this.sessions.set(sessionId, { channel, createdAt: Date.now(), messageCount: 0 });
  ...
  async chat(sessionId: string, text: string): Promise<string> {
    ...
    await delay(10);
    bus.emit("tool:start", { sessionId, tool: "desktop_screenshot", input: { displayId: 0 } });
    await delay(10);
  ...
  getStatus() {
    return {
      ...
      globalCap: 10,
      channels: [
        { channel: "computer", active: this.sessions.size, cap: 3, idleTimeoutMs: 600_000 },
      ],
    };
  }
}
  ...
  try {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === "ack" && msg.payload?.type === "session:created") {
        ...
        if (msg.payload.channel !== "computer") fail(`expected channel=computer, got ${msg.payload.channel}`);
        ws.send(JSON.stringify({ type: "intent", text: "screenshot my desktop" }));
      }

      if (msg.type === "event") {
        const ev = msg.event;
        console.log(`[smoke] event: ${ev.kind}${ev.tool ? ` tool=${ev.tool}` : ""}${ev.final ? " final" : ""}`);
    ...
    const finalToken = events.find((e) => e.type === "event" && e.event.kind === "token" && e.event.final);
    ...
    for (const e of events) {
      console.log(`  ${e.type}${e.event ? `:${e.event.kind}` : ""}${e.payload ? ` ${JSON.stringify(e.payload).slice(0, 80)}` : ""}`);
    }
