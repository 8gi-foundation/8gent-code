...
export type DaemonChannel = "os" | "app" | "telegram" | "discord" | "api" | "delegation" | "computer";
...
export type StreamEvent =
  ...
  | { kind: "tool_call"; sessionId: string; tool: string; input: unknown; callId: string }
  | { kind: "tool_result"; sessionId: string; tool: string; output: unknown; callId: string; durationMs: number }
  | { kind: "approval_required"; sessionId: string; tool: string; input: unknown; requestId: string; reason?: string }
  | { kind: "error"; sessionId: string; error: string; recoverable?: boolean }
  | { kind: "done"; sessionId: string; reason: "turn-complete" | "client-disconnect" | "client-destroy" | "idle-timeout" | "channel-cap-evict" };
