...
export function extractBranchName(output: string): string | null {
	...
	const match = output.match(/On branch (\S+)/) || output.match(/\* (\S+)/);
...
export function formatForVoice(report: CompletionReport): string {
	...
	if (totalFiles > 0) {
		if (report.filesCreated.length > 0 && report.filesModified.length > 0) {
			parts.push(
				`Created ${report.filesCreated.length} and modified ${report.filesModified.length} files.`,
			);
		} else if (report.filesCreated.length > 0) {
			parts.push(
				`Created ${report.filesCreated.length} file${report.filesCreated.length > 1 ? "s" : ""}.`,
			);
		} else {
			parts.push(
				`Modified ${report.filesModified.length} file${report.filesModified.length > 1 ? "s" : ""}.`,
			);
		}
	}
