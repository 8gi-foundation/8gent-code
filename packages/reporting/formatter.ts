...
export function table(headers: string[], rows: string[][], options: TableOptions = {}): string {
	...
	// Header separator
	const headerSep =
		colorize(boxChars.teeRight, borderColor) +
	...
	// Bottom border
	const bottomBorder =
		colorize(boxChars.singleBottomLeft, borderColor) +
...
export function tree(items: TreeItem[], indent = 0): string {
	...
	items.forEach((item, index) => {
		...
		lines.push(
			`${prefix}${muted(connector + boxChars.singleHorizontal + boxChars.singleHorizontal)} ${item.label}`,
		);
