...
async function checkOpenRouterVision(apiKey?: string): Promise<VisionModel[]> {
	try {
		...
		for (const m of models) {
			...
			const isOcr =
				/ocr|dots\.ocr|document-parse/i.test(m.id) ||
...
export async function findVisionModel(options?: {
	...
	if (!config.enabled) {
		return {
			...
			model: null,
			allAvailable: [],
			error: "Vision is disabled in config. Enable with /vision on",
		};
	}
...
export function getRecommendedOCRModels(): Array<{
	...
	return [
		{
			model: "deepseek-ocr",
			description: "DeepSeek vision-language OCR",
			size: "~2GB",
		},
		{
			model: "glm-ocr",
			description: "GLM multimodal document understanding (fastest)",
			size: "~1GB",
		},
		{
			model: "dots.ocr",
			description: "SOTA multilingual OCR — 83.9 on olmOCR-bench",
			size: "~3GB",
		},
		{
			model: "qwen2.5-vl",
			description: "General vision + strong OCR (native resolution)",
			size: "~5GB",
		},
		{
			model: "minicpm-v",
			description: "Mobile-friendly vision + OCR",
			size: "~5GB",
		},
	];
}
