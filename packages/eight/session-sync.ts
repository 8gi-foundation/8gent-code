...
export class SessionSyncManager {
	...
	async flush(): Promise<void> {
		...
		if (
			this.pending.tokensIn === 0 &&
			this.pending.tokensOut === 0 &&
			this.pending.toolCalls === 0
		) {
	...
	startCheckpointTimer(getMessages: () => Array<{ role: string; content: string }>): void {
		...
		if (
			...
			typeof this.checkpointTimer === "object" &&
			"unref" in this.checkpointTimer
		) {
	 ...
	 */
	async checkActiveConflicts(): Promise<
		Array<{ id: string; channel: string; model: string; startedAt: number }>
	> {
		if (!this.enabled || !this.userId) return [];
	...
	async claimRemoteSession(sessionId: string): Promise<boolean> {
		...
		try {
			...
			const result = await this._client.mutation(this._api.sessions.claim, {
				sessionId,
