...
`);
	...
	const prompt = () => {
		rl.question("\n\x1b[36m❯\x1b[0m ", async (input) => {
			...
			// Model commands
			if (await handleModelCommands(trimmed, agent)) {
				prompt();
				return;
			}

			// Provider commands
			if (await handleProviderCommands(trimmed)) {
				prompt();
				return;
			}

			// Planner/board commands
			if (handlePlannerCommands(trimmed)) {
				prompt();
				return;
			}

			// Plan/status commands
			if (await handlePlanCommands(trimmed, agent)) {
				prompt();
				return;
			}

			// MCP commands
			if (handleMCPCommands(trimmed)) {
				prompt();
				return;
			}

			// LSP commands
			if (handleLSPCommands(trimmed)) {
				prompt();
				return;
			}

			// Report commands
			if (handleReportSlashCommands(trimmed, agent)) {
				prompt();
				return;
			}

			// Voice commands
			if (await handleVoiceCommands(trimmed)) {
				prompt();
				return;
			}

			// Language commands
			if (await handleLanguageCommands(trimmed)) {
				prompt();
				return;
			}

			// Personal LoRA commands
			if (handlePersonalCommands(trimmed)) {
				prompt();
				return;
			}

			// Background task commands
			if (handleBackgroundCommands(trimmed)) {
				prompt();
				return;
			}

			// Permission commands
			if (handlePermissionCommands(trimmed)) {
				prompt();
				return;
			}

			// Hooks commands
			if (handleHooksCommands(trimmed)) {
				prompt();
				return;
			}

			// Skills commands
			if (await handleSkillsCommands(trimmed)) {
				prompt();
				return;
			}

			// Multi-agent commands
			if (await handleAgentCommands(trimmed)) {
				prompt();
				return;
			}

			// Task commands
			if (handleTaskCommands(trimmed)) {
				prompt();
				return;
			}

			// Telegram commands
			if (await handleTelegramCommands(trimmed, agent, rl)) {
				prompt();
				return;
			}

			// Secrets commands
			if (handleSecretsCommands(trimmed)) {
				prompt();
				return;
			}

			// Skill invocation (/<skill-name>)
			if (await handleSkillInvocation(trimmed, agent)) {
				prompt();
				return;
			}
	...
	if (trimmed === "/predict") {
		...
		if (ready.length === 0 && !next) {
			...
			if (next) {
				...
				console.log(
					`    Tool: ${next.tool} | Confidence: ${Math.round(next.confidence * 100)}% | Priority: ${next.priority}\n`,
				);
			}
			if (ready.length > 0) {
...
async function handleSkillsCommands(trimmed: string): Promise<boolean> {
	if (trimmed === "/skills") {
		try {
			...
			if (skills.length === 0) {
				console.log(
					"  No skills found. Add .md files in ~/.8gent/skills/ or run from a repo with .claude/skills/*/SKILL.md",
				);
			}
		} catch (err) {
...
async function handleAgentCommands(trimmed: string): Promise<boolean> {
	if (trimmed.startsWith("/spawn ")) {
		...
		if (!parsed || !parsed.task) {
			console.log(
				"\x1b[31mUsage: /spawn <task description> [--model <model>] [--dir <path>]\x1b[0m",
			);
			return true;
		}
	...
	if (trimmed.startsWith("/kill ")) {
		...
		if (killed) {
			...
			console.log(
				`\x1b[31mCould not kill agent ${agentId} (not found or already completed)\x1b[0m`,
			);
		}
		return true;
	}
	...
	if (trimmed === "/evidence") {
		try {
			...
			if (totalEvidence.length === 0) {
				...
				console.log(
					`\x1b[36mTotal:\x1b[0m ${summary.total} items | ${summary.verified} verified | ${summary.failed} failed`,
				);
			}
		} catch (err) {
			console.error(`\x1b[31mError: ${err}\x1b[0m`);
