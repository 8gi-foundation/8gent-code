 ...
 *
 * Syntax:
 *   {{variable}}                        - interpolation (dot-paths ok)
...
export interface TemplateOptions {
  /** Throw on unresolved variables / missing partials instead of emitting empty string */
  strict?: boolean;
...
export interface ValidationResult {
  valid: boolean;
  errors: string[];
  /** All variable paths referenced in the template */
  variables: string[];
  /** All partial names referenced in the template */
  partials: string[];
}
...
function renderTokens(
  ...
  if (depth > 64) throw new Error("PromptTemplate: max nesting depth (64) exceeded");
  let output = "";
  let i = 0;
  while (i < tokens.length) {
    ...
    switch (token.kind) {
      ...
      case "if_open": {
        const inner = renderTokens(tokens.slice(i + 1), stack, options, depth + 1);
        if (isTruthy(resolve(token.value, stack))) output += inner.output;
        i += inner.consumed + 2;
        break;
      }
      case "unless_open": {
      ...
      case "each_open": {
        ...
        const probe = renderTokens(bodyTokens, stack, options, depth + 1);
      ...
      case "each_close":
        return { output, consumed: i };
      case "partial": {
        const src = options.partials[token.value];
        if (!src) {
          if (options.strict)
            throw new Error(`PromptTemplate: partial "{{> ${token.value}}}" not registered`);
          i++;
          break;
        }
        const pt = new PromptTemplate(src, options);
        output += pt.render(stack.reduce((acc, s) => ({ ...acc, ...s }), {}));
        i++;
        break;
      }
      default:
...
export class PromptTemplate {
  ...
  render(context: TemplateContext = {}): string {
    return renderTokens(this.tokens, [context], this.options, 0).output;
  }

  /**
   * Validate template structure without rendering.
   * Returns all syntax errors and the list of referenced variable names.
   */
  validate(): ValidationResult {
    const errors: string[] = [];
    const variables = new Set<string>();
    const partialRefs = new Set<string>();
    let ifDepth = 0, unlessDepth = 0, eachDepth = 0;
    for (const token of this.tokens) {
      switch (token.kind) {
        ...
        case "if_open":
          ifDepth++;
          if (!token.value) errors.push("{{#if}} missing condition expression");
        ...
        case "unless_open":
          unlessDepth++;
          if (!token.value) errors.push("{{#unless}} missing condition expression");
          else variables.add(token.value.split(".")[0]);
          break;
        case "unless_close":
        ...
        case "each_open":
          eachDepth++;
          if (!token.value) errors.push("{{#each}} missing array expression");
        ...
        case "partial":
          ...
          if (this.options.strict && !(token.value in this.options.partials))
            errors.push(`Partial "{{> ${token.value}}}" not registered`);
          break;
      }
    }
  ...
  /** Register a partial - returns a new instance (original unchanged). */
...
/**
 * Agent system prompt.
 * Required: agent_name, model, capabilities (array of {name, description}), user_name
 * Optional: context, memory_snippets (array of strings), cwd, date
 */
...
{{/each}}
{{/if}}
Be concise. Show your work. Prefer the smallest change that solves the problem.`
);

/**
 * Sub-agent task delegation.
 * Required: task, parent_agent, tools (array of {name, description})
 * Optional: deadline, priority, notes
 */
...
{{notes}}
{{/if}}
Report results as structured JSON with keys: status, output, errors, duration_ms.`
);

/**
 * Post-session reflection and self-evaluation.
 * Required: session_id, goal, actions (array of {tool, result})
 * Optional: duration_ms, tokens_used
 */
...
{{#if tokens_used}}Tokens used: {{tokens_used}}{{/if}}

## Actions
{{#each actions}}{{@index}}. {{tool}} -> {{result}}
{{/each}}

Evaluate this session. Return JSON with:
- goal_achieved: boolean
- quality_score: 0-100
- key_learnings: string[]
- improvements: string[]
- skill_updates: {skill: string, delta: number}[]`
);

/**
 * Memory injection segment - insert into any system prompt.
 * Required: memories (array of {content, relevance_score, type})
 */
export const MEMORY_INJECTION_TEMPLATE = new PromptTemplate(
`## Recalled Memory (by relevance)
 ...
 * Required: error_message, failed_action, context
 * Optional: attempts, max_attempts, suggestions (array of strings)
 */
...
{{/each}}
{{/if}}
Diagnose the root cause and propose the single smallest fix.`
);

/**
 * Code review request.
 * Required: diff, repo, branch
 * Optional: pr_title, pr_description, checklist (array of strings)
 */
...
{{/each}}
{{/if}}
Review for: correctness, security, performance, style. Return structured JSON.`
);
...
/** Render a named built-in template. */
export function renderBuiltin(name: keyof typeof BUILT_IN_TEMPLATES, context: TemplateContext): string {
  ...
  if (!tpl) throw new Error(`PromptTemplate: unknown built-in "${String(name)}"`);
