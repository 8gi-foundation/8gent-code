...
export function formatContextSummary(summary: ContextSummary): string {
	...
	if (summary.files.length > 0) {
		parts.push(
			`Files: ${summary.files.slice(0, 5).join(", ")}${summary.files.length > 5 ? ` (+${summary.files.length - 5})` : ""}`,
		);
	}

	if (summary.symbols.length > 0) {
		parts.push(
			`Symbols: ${summary.symbols.slice(0, 5).join(", ")}${summary.symbols.length > 5 ? ` (+${summary.symbols.length - 5})` : ""}`,
		);
	}
