	...
	steps: [
		{
			...
			objective:
				"Understand the high-level architecture: CLI -> agent loop -> tools -> LLM providers.",
			prerequisites: [],
			content: [
			...
			verifyUnderstanding:
				"What are the three LLM runtimes Eight supports, and where is the runtime selected?",
			estimatedMinutes: 8,
		},
		{
			...
			objective:
				"Understand how Eight processes a user message: prompt assembly, LLM call, tool dispatch, and iteration.",
		...
		},
		{
			...
			verifyUnderstanding:
				"Why does Eight use a single `run()` tool instead of many individual tools?",
			estimatedMinutes: 10,
		},
		{
			id: "arch-4-providers",
			...
			verifyUnderstanding:
				"If you wanted to add a new LLM provider (e.g., Anthropic direct), what interface methods would you implement?",
			estimatedMinutes: 8,
		},
		{
			...
			verifyUnderstanding:
				"What's the difference between a 'transient' and 'recoverable' error in Eight's classification?",
			estimatedMinutes: 9,
		},
	],
};
	...
	steps: [
		{
			...
			objective:
				"Understand the structure of a benchmark: prompt, expected output, grading criteria.",
			prerequisites: [],
			content: [
			...
			verifyUnderstanding:
				"What's the difference between a 'test' and a 'fixture' in the benchmark system?",
			estimatedMinutes: 8,
		},
		{
			id: "bench-2-writing",
			...
			verifyUnderstanding:
				"Why should benchmark tests verify behavior rather than exact string output?",
			estimatedMinutes: 10,
		},
		{
			id: "bench-3-grading",
			...
			objective:
				"Understand how execution-grader.ts scores benchmarks and how to debug failing tests.",
			...
			verifyUnderstanding:
				"What happens when the grader can't extract any files from the LLM response?",
			estimatedMinutes: 7,
		},
		{
			...
			objective:
				"Understand how the autoresearch loop uses benchmark results to improve the system prompt.",
