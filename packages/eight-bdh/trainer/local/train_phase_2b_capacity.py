"""
Phase 2b capacity: 5M BDH on a 5-8MB harness corpus.
...
Outputs:
  packages/eight-bdh/checkpoints/phase-2b-capacity-10m.pt
  packages/eight-bdh/data/phase-2b-capacity-corpus.bin
  packages/eight-bdh/trainer/local/phase-2b-capacity-train-log.json
...
MAX_ITERS = int(os.environ.get("BDH_MAX_ITERS", "2000"))
...
print(f"[boot] config 10M: n_embd={MODEL_CFG.n_embd} mlp_mult={MODEL_CFG.mlp_internal_dim_multiplier}", flush=True)
...
data_path = DATA_DIR / "phase-2b-capacity-corpus.bin"
...
print(f"[model] params={n_params/1e6:.2f}M target=10M", flush=True)
...
# Save checkpoint
checkpoint_path = CHECKPOINT_DIR / "phase-2b-capacity-10m.pt"
torch.save(
    {
        ...
        "phase": 2,
        "subphase": "a-scale",
        "model_id": "8gent-0.1.0-bdh-r:10m-phase-2b-capacity",
    },
    checkpoint_path,
)
print(f"[save] checkpoint written to {checkpoint_path.relative_to(REPO_ROOT)}", flush=True)

log_path = LOG_DIR / "phase-2b-capacity-train-log.json"
