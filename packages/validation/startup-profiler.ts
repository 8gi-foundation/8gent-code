...
function isColdStart(dataDir: string): boolean {
  ...
  try {
    ...
    // Cold if last run was more than 5 minutes ago
...
function traceImports(
  ...
  // Use bun's --smol flag + timing to measure startup
  ...
  // Parse slow imports from build output (bun build logs resolve times)
