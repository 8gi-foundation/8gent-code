...
function normalizeLines(lines: string[]): string[] {
  return lines.map((l) => l.trim()).filter((l) => l.length > 0 && !l.startsWith("//") && !l.startsWith("#"));
}
...
function findSimilarBlocks(
  ...
  // Map: normalized-block-hash -> [{file, startLine, snippet}]
  const blockMap = new Map<
    string,
    Array<{ file: string; startLine: number; snippet: string }>
  >();
  ...
  for (const file of codeFiles) {
    ...
    for (let i = 0; i <= normalized.length - minLines; i++) {
      ...
      // Find approximate raw line number
      const snippet = window.slice(0, 3).join("\n");
      entry.push({ file, startLine: i + 1, snippet });
  ...
  for (const [, locations] of blockMap) {
    ...
    // Deduplicate same file same location
  ...
  // Sort by number of duplicate locations descending
  results.sort((a, b) => b.locations.length - a.locations.length);

  // Deduplicate results that are subsets of each other by snippet prefix
  ...
  for (const r of results) {
    ...
    final.push(r);
    if (final.length >= 50) break; // cap output
  }
  return final;
}
...
export async function findDuplicates(
  ...
  const totalWastedBytes = duplicateGroups.reduce(
    (acc, g) => acc + g.wastedBytes,
    0
  );
...
async function main() {
  ...
  if (report.duplicateGroups.length > 0) {
    ...
    if (report.duplicateGroups.length > 20) {
      console.log(`  ... and ${report.duplicateGroups.length - 20} more groups`);
    }
  }
  ...
  if (report.similarBlocks.length > 0) {
    ...
    if (report.similarBlocks.length > 15) {
      console.log(`  ... and ${report.similarBlocks.length - 15} more blocks`);
    }
  }
}
