...
describe("Policy Engine", () => {
  beforeEach(() => {
    // Reload defaults fresh each test
    loadPolicies();
  });
  ...
  test("blocks writing to .env files (default policy)", () => {
    ...
    // Default policies should block writing secrets
    // If no matching rule, engine should still return a decision
    expect(decision).toHaveProperty("allowed");
  });
  ...
  test("require_approval decision includes requiresApproval flag", () => {
    addPolicy({
      name: "approve-push",
      action: "git_push",
