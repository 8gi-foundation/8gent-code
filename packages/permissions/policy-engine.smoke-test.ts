...
function assert(
	...
	console.log(
		`  [${ok ? "PASS" : "FAIL"}] ${label}: allowed=${decision.allowed}`,
	);
	if (!ok) fail++;
	else pass++;
}
...
assert(
	...
	checkFileWrite("src/utils.ts", "export const x = 1;"),
	true,
);
...
console.log(
	`  [${addPolicyAllowed ? "PASS" : "FAIL"}] addPolicy allows non-conflicting rules`,
);
