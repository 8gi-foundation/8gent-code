...
export class AccessAuditStore {
  ...
  queryAccess(options: QueryAccessOptions = {}): AccessEvent[] {
    ...
    if (options.targetTable) { clauses.push("target_table = ?"); params.push(options.targetTable); }
    if (options.actor)       { clauses.push("actor = ?");        params.push(options.actor); }
    if (typeof options.since === "number") { clauses.push("created_at >= ?"); params.push(options.since); }
    if (typeof options.until === "number") { clauses.push("created_at <= ?"); params.push(options.until); }
    ...
    const rows = this.db
      ...
      .all(...params, limit) as Array<{
        id: string; created_at: number; actor: string; actor_kind: string;
        target_table: string; target_id: string; operation: string;
        reason: string; session_id: string | null;
      }>;
  ...
  count(): number {
    const row = this.db.prepare(`SELECT COUNT(*) AS n FROM access_audit_log`).get() as { n: number };
    return row.n;
  }
