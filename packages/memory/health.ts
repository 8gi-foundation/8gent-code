...
export function memoryHealth(db: Database): MemoryHealth {
	const total =
		(db.prepare("SELECT COUNT(*) as count FROM memories WHERE deleted_at IS NULL").get() as any)
			?.count ?? 0;
	const stale =
		(
			db
	...
	const oldest = (
		db.prepare("SELECT MIN(created_at) as ts FROM memories WHERE deleted_at IS NULL").get() as any
	)?.ts;
	const newest = (
		db.prepare("SELECT MAX(created_at) as ts FROM memories WHERE deleted_at IS NULL").get() as any
	)?.ts;
