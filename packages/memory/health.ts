...
export function memoryHealth(db: Database): MemoryHealth {
	...
	const stale =
		(
			db
				.prepare(
					"SELECT COUNT(*) as count FROM memories WHERE deleted_at IS NULL AND importance < 0.3",
				)
				.get() as any
		)?.count ?? 0;
	const avgStrength =
		(
			db
				.prepare(
					"SELECT AVG(importance) as avg FROM memories WHERE deleted_at IS NULL",
				)
				.get() as any
		)?.avg ?? 0;
	const oldest = (
		db
			.prepare(
				"SELECT MIN(created_at) as ts FROM memories WHERE deleted_at IS NULL",
			)
			.get() as any
	)?.ts;
	const newest = (
		db
			.prepare(
				"SELECT MAX(created_at) as ts FROM memories WHERE deleted_at IS NULL",
			)
			.get() as any
	)?.ts;
	...
	try {
		const row = db
			.prepare("SELECT MAX(created_at) as ts FROM consolidation_log")
			.get() as any;
	...
	if (total > 0) {
		...
		if (
			lastConsolidation &&
			Date.now() - new Date(lastConsolidation).getTime() > SEVEN_DAYS_MS
		) {
			score -= 20;
