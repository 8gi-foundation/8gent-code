...
describe("update() — versioning", () => {
  ...
  it("creates sequential version snapshots on multiple updates", () => {
    ...
    const rawDb = new Database(dbPath, { readonly: true });
    const versions = rawDb
      .prepare("SELECT version, data_snapshot, change_reason FROM memory_versions WHERE memory_id = ? ORDER BY version")
  ...
  it("updates the data blob stored in the memories table", () => {
    ...
    const rawDb = new Database(dbPath, { readonly: true });
    const row = rawDb.prepare("SELECT data, version, importance FROM memories WHERE id = ?").get("mem_merged") as {
      data: string;
      version: number;
      importance: number;
    } | null;
...
describe("forget + update interplay", () => {
  it("update creates snapshot, then forget creates another snapshot", () => {
    ...
    const rawDb = new Database(dbPath, { readonly: true });
    const versions = rawDb
      .prepare("SELECT version, change_reason FROM memory_versions WHERE memory_id = ? ORDER BY version")
