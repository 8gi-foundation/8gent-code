...
describe("lintMemory", () => {
  ...
  it("clean state returns high score", () => {
    ...
    db.prepare("INSERT INTO consolidation_log (id, created_at) VALUES (?, ?)").run(
      "clog_1",
      now
    );
  ...
  it("detects orphan entities", () => {
    ...
    insertMemory(db, { content_text: "some basic memory content", importance: 0.7 });
  ...
  it("score penalties accumulate", () => {
    ...
    db2.prepare("INSERT INTO consolidation_log (id, created_at) VALUES (?, ?)").run(
      "clog_1",
      now
    );
