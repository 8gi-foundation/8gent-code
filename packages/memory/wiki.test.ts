...
describe("WikiGenerator", () => {
  beforeEach(() => {
    ...
    // Seed entities
    personAlice = graph.addEntity("person", "Alice", { description: "Lead engineer" });
    personBob = graph.addEntity("person", "Bob", { description: "Backend developer" });
    conceptAuth = graph.addEntity("concept", "Authentication", { description: "User identity verification" });
    conceptCache = graph.addEntity("concept", "Caching");
    serviceRedis = graph.addEntity("tool", "Redis", { description: "In-memory data store" });
