...
describe("SharedMemoryBus", () => {
  ...
  it("stores messages and retrieves them in chronological order", () => {
    ...
    bus.storeMessage("hello", "user", { source: "discord", scope, authorId: "u1", authorName: "James" });
    bus.storeMessage("hi there", "assistant", { source: "discord", scope });
    bus.storeMessage("how are you?", "user", { source: "discord", scope, authorId: "u1", authorName: "James" });
  ...
  it("returns cross-scope messages for a channel prefix", () => {
    bus.storeMessage("msg from EO", "user", { source: "discord", scope: "discord:ch1:8EO" });
    bus.storeMessage("msg from TO", "user", { source: "discord", scope: "discord:ch1:8TO" });
    bus.storeMessage("msg from other channel", "user", { source: "discord", scope: "discord:ch2:8EO" });
  ...
  it("correctly handles two-segment scopes without trimming", () => {
    bus.storeMessage("telegram msg", "user", { source: "telegram", scope: "telegram:chat-456" });
