...
import { KnowledgeGraph, type Entity, type EntityType, type Relationship } from "./graph.js";
...
export class WikiGenerator {
  ...
  generateEntityPage(entityId: string): WikiPage | null {
    ...
    if (memories.length > 0) {
      ...
      for (const mem of memories) {
        const snippet = mem.content_text.length > 200
  ...
  writeToDirectory(outputDir: string): { pagesWritten: number; indexWritten: boolean } {
