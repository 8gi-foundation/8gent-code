...
import { type CoreMemory, type SemanticMemory, type Memory, generateId } from "./types.js";
...
describe("JSONB double-encoding guard", () => {
  ...
  it("heals double-encoded data on read", () => {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id, mem.type, mem.scope, doubleEncoded, contentText,
      JSON.stringify(mem.tags), mem.importance, mem.decayFactor,
      mem.accessCount, mem.lastAccessed, mem.confidence, mem.evidenceCount,
      mem.version, mem.source, null, now, now
    );
  ...
  it("handles 2 levels of double-encoding on read", () => {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id, mem.type, mem.scope, tripleEncoded, contentText,
      JSON.stringify(mem.tags), mem.importance, mem.decayFactor,
      mem.accessCount, mem.lastAccessed, mem.confidence, mem.evidenceCount,
      mem.version, mem.source, null, now, now
    );
