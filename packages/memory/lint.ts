...
export function lintReportToMarkdown(report: LintReport): string {
  ...
  if (report.contradictions.length > 0) {
    ...
    for (const c of report.contradictions) {
      ...
      lines.push(`- [${c.conflictType}] "${snipA}" vs "${snipB}" (confidence: ${c.confidence.toFixed(2)})`);
    }
    lines.push("");
  }
  ...
  if (report.stale.length > 0) {
    ...
    for (const s of top10) {
      const days = s.daysSinceAccess === Infinity ? "never accessed" : `${s.daysSinceAccess}d ago`;
      lines.push(`- \`${s.id}\` importance=${s.importance.toFixed(2)} last=${days} — ${s.content.slice(0, 60)}`);
    }
    lines.push("");
  }
  ...
  if (report.broken.length > 0) {
    ...
    for (const b of report.broken) {
      lines.push(`- Relationship \`${b.relationship.id}\`: ${b.reason} (source=${b.relationship.sourceId}, target=${b.relationship.targetId})`);
    }
    lines.push("");
  }
