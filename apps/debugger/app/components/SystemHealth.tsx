		...
		<div className="h-full overflow-y-auto p-4 space-y-6">
			...
			<div className="flex items-center justify-between">
				...
				<span className="text-[10px]" style={{ color: "var(--muted)" }}>
					uptime: {formatUptime(data.uptime)} · refreshed{" "}
				...
				>
					...
					<div className="flex items-center gap-4 mb-3">
						...
						<div className="flex-1">
							...
							<ProgressBar
								value={data.evidence.verified}
								max={data.evidence.total || 1}
								color="emerald"
							/>
						</div>
					</div>
					{data.evidence.failed > 0 && (
						<div className="text-[10px] text-red-400/60 mt-1">
							{data.evidence.failed} failed verification
							{data.evidence.failed !== 1 ? "s" : ""}
						</div>
					)}
					...
					{[
						{
							...
							status: data.ast.filesIndexed > 0,
							detail: `${data.ast.filesIndexed} files`,
						},
						{
							name: "BMAD Planning",
							status: true,
							detail: "Universal (5 types)",
						},
						{
							name: "Multi-Agent Orchestration",
							status: true,
							detail: "Pool + Subagents",
						},
						{
