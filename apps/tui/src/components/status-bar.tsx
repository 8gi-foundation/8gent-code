...
function TokenSavingsItem({
	...
	return (
		<Inline gap={0}>
			...
			<Label color={savingsColor}>{percentage}%</Label>
			<MutedText> {tokenBit}</MutedText>
		</Inline>
	);
}
...
function AuthStatusItem({
	...
	if (status === "authenticated" && user) {
		...
		return <Badge label={`\u2713 ${short}`} color="green" variant="outline" />;
	}
...
function PlanStatusItem({
	...
	const configs = {
		...
		planning: {
			...
			icon: "\u25D4",
			label: "planning...",
			verbType: "planning" as StatusVerbType,
		},
		executing: {
			color: "yellow" as const,
			icon: "\u25B6",
			label: `${completed}/${total}`,
			verbType: "executing" as StatusVerbType,
		},
		completed: {
			color: "green" as const,
			icon: "\u2713",
			label: "ready (awaiting input)",
			verbType: null,
		},
	};
...
function TokenCounter({ value, animate = true }: TokenCounterProps) {
	...
	return (
		<Box>
			...
			<Label color="green">{formattedValue}</Label>
...
function HelpText({ animate = true, tick = 0 }: HelpTextProps) {
	...
	return <MutedText>{hints[currentHint]}</MutedText>;
}
...
export function CompactStatusBar({ tokensSaved }: StatusBarProps) {
	return (
		...
		>
			...
			<Label color="green">{formatTokens(tokensSaved)} saved</Label>
