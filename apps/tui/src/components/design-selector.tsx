...
export function DesignSelector({
	...
	return (
		<Card borderColor="magenta">
			{/* Header */}
			<Box marginBottom={1}>
				<Label color="magenta">{pulseChar} Design Direction</Label>
			</Box>
		...
		>
			...
			<Inline justifyContent="space-between">
				<Inline gap={0}>
					...
					<Label color={isSelected ? "cyan" : undefined}> {option.name}</Label>
				</Inline>
				<MutedText>{Math.round(option.score)}% match</MutedText>
			</Inline>
			...
			{/* Reasoning */}
			<Box marginTop={0}>
				<SuccessText italic>\u2192 {option.reasoning}</SuccessText>
			</Box>
...
function DesignConfirmation({
	...
	return (
		<Card borderColor="green">
			<Box marginBottom={1}>
				<Label color="green">\u2713 Confirm Selection</Label>
			</Box>
			...
			<Box marginTop={1}>
				<MutedText>Press [Enter/Y] to confirm, [ESC/N] to go back</MutedText>
			</Box>
		</Card>
	);
}
...
export function InlineDesignPrompt({
	...
	return (
		<Stack marginY={1}>
			<Inline gap={0}>
				<Label color="magenta">{paintbrushIcon} Design Agent</Label>
			</Inline>
			<Box paddingLeft={2}>
				<Label>{message}</Label>
			</Box>
