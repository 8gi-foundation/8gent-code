...
import { formatDuration as formatDurationLib, formatTokens } from "../lib/format.js";
...
export function CompletionReport({
	...
	return (
		<Stack paddingX={1}>
			...
			{/* Files Section */}
			{revealedSections >= 3 &&
				(report.filesCreated.length > 0 || report.filesModified.length > 0) && (
					<CollapsibleSection
	...
	return (
		<Stack marginBottom={1}>
			<Text color={color}>
				...
				{boxChars.doubleHorizontal.repeat(width)}
...
function Section({ title, children }: SectionProps) {
	return (
		<Stack marginBottom={1}>
			...
			<Stack marginLeft={2}>{children}</Stack>
		</Stack>
	);
}
...
function CollapsibleSection({
	...
	return (
		<Stack marginBottom={1}>
			...
			{expanded && <Stack marginLeft={2}>{children}</Stack>}
		</Stack>
	);
}
...
function StatsGrid({ report }: { report: CompletionReportData }) {
	...
	if (report.tokensUsed) {
		stats.push({
			label: "Tokens used",
			value: formatTokens(report.tokensUsed),
			dim: true,
		});
	}
	...
	if (report.tokensSaved) {
		stats.push({
			label: "Tokens saved",
			value: formatTokens(report.tokensSaved),
			color: "green",
		});
	}
	...
	if (report.gitCommit) {
		stats.push({
			label: "Commit",
			value: report.gitCommit.slice(0, 7),
			color: "cyan",
		});
	}
