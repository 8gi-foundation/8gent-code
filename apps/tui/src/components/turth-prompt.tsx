...
import { AppText, MutedText, Heading, Card, Stack } from "./primitives/index.js";
import {
...
const SCOPE_OPTIONS: SelectOption<ApprovalScope>[] = [
  { label: "Allow once", value: "once", description: "This call only. Nothing is cached." },
  { label: "Allow for session", value: "session", description: "Remember until the TUI exits." },
  { label: "Allow for this project", value: "project", description: "Remember for this working directory." },
  { label: "Allow always", value: "always", description: "Persist across restarts. Stored in ~/.8gent/user-policy.json." },
  { label: "Deny", value: "deny", description: "Reject this call. Re-prompt next time." },
];
    ...
    <Box marginTop={1} flexDirection="column">
      <Card borderColor="yellow">
        <Stack gap={0}>
          ...
          {pending.detail ? (
            <MutedText>{pending.detail}</MutedText>
          ) : null}
