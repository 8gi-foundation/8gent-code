	...
	return (
		<Card borderColor="cyan">
			...
			<Inline justifyContent="space-between" marginBottom={1}>
				<Heading>{"\u2592"} Plan Kanban Board</Heading>
				<MutedText>[ESC] close</MutedText>
			</Inline>
	...
	return (
		...
		>
			<Inline gap={0} justifyContent="space-between" flexGrow={1}>
				<Label color={color as any}>{displayTitle}</Label>
...
function KanbanCard({
	...
	return (
		<Stack paddingX={1} marginBottom={1} {...cardStyle}>
			...
			{/* Confidence */}
			<MutedText>{Math.round(step.confidence * 100)}% conf</MutedText>
		</Stack>
	);
}
	...
	return (
		<Card borderColor="magenta">
			...
			<Inline gap={0} marginBottom={1}>
				<Heading color="magenta">{"\u2263"} Possible Avenues</Heading>
				<MutedText> ({avenues.length} paths)</MutedText>
			</Inline>
			...
			{/* Help */}
			<Box marginTop={1}>
				<MutedText>Type to match avenue or use /avenue [number]</MutedText>
			</Box>
		</Card>
	);
}
...
export function PredictedSteps({
	...
	return (
		<Card borderColor="green">
			{/* Header */}
			<Box marginBottom={1}>
				<Heading color="green">{"\u25B6"} Predicted Next Steps</Heading>
			</Box>
			...
			{/* Help */}
			<Box marginTop={1}>
				<MutedText>Press [Tab] on input to accept top prediction</MutedText>
			</Box>
		</Card>
	);
}
...
export function AutoPlanKanban({
	...
	return (
		<Card borderColor="cyan">
			...
			{/* Footer with stats */}
			<Inline justifyContent="space-between" marginTop={1}>
				<MutedText>{stats.total} tasks</MutedText>
				<Inline gap={0}>
...
function AutoKanbanCardView({ card, compact, width }: AutoKanbanCardViewProps) {
	...
	if (compact) {
		return (
			<Inline gap={0} paddingX={1}>
				<AppText color={statusColor as any}>{card.icon}</AppText>
	...
	return (
		<Stack paddingX={1} marginBottom={0}>
			...
			<Inline gap={0} justifyContent="space-between">
				<MutedText>[{truncate(card.assignedTabName, 12)}]</MutedText>
				{durationStr && <MutedText>{durationStr}</MutedText>}
				{card.status === "failed" && <AppText color="red">{"\u2717"}</AppText>}
			</Inline>
		</Stack>
	);
}
