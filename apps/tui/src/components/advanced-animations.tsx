	...
	const lines = useMemo(() => {
		...
		for (let y = 0; y < height; y++) {
			...
			for (let x = 0; x < width; x++) {
				...
				if (x === x1) {
					line.push(
						<Text key={x} color={baseColors[baseIndex]} bold>
							{bases[baseIndex]}
						</Text>,
					);
				} else if (x === x2) {
					line.push(
						<Text key={x} color={baseColors[(baseIndex + 2) % 4]} bold>
							{bases[(baseIndex + 2) % 4]}
						</Text>,
					);
				} else if (
