	...
	return (
		<Card borderColor="cyan">
			...
			<Stack height={maxHeight}>
				{evidence.length === 0 ? (
					<MutedText italic>No evidence collected yet.</MutedText>
				) : (
			...
			{/* Navigation hint */}
			<Box marginTop={1}>
				<MutedText>↑↓ navigate • Enter to expand • q to close</MutedText>
			</Box>
		</Card>
	);
}
...
function EvidenceDataPreview({
	...
	if (typeof data === "string") {
		...
		return <MutedText wrap="truncate">"{preview}..."</MutedText>;
	}
...
export function ConfidenceMeter({
	...
	return (
		<Inline gap={0}>
			{showLabel && <MutedText>Confidence: </MutedText>}
				...
				{steps.map((step, index) => {
					...
					return (
						<Inline key={step.stepId} gap={0}>
							...
							{step.duration && <MutedText> ({step.duration}ms)</MutedText>}
						</Inline>
					);
				})}
			</Stack>
	...
	return (
		<Stack padding={1}>
			...
			<Inline marginBottom={1}>
				<Tab label="1. Summary" active={activeTab === "summary"} />
				<Tab label="2. Steps" active={activeTab === "steps"} />
				<Tab label="3. Evidence" active={activeTab === "evidence"} />
			</Inline>

			{/* Tab content */}
			{activeTab === "summary" && <SummaryTab report={report} />}
			{activeTab === "steps" && <StepPanel steps={report.steps} />}
			{activeTab === "evidence" && <EvidencePanel evidence={report.evidence} />}
		</Stack>
	);
}
...
function SummaryTab({
	...
	return (
		<Card borderColor="cyan">
			<Heading>Validation Summary</Heading>
