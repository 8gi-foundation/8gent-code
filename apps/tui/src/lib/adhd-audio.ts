...
const SOUNDSCAPES: Record<ADHDSoundscape, SoundscapePreset> = {
	lofi: {
		prompt:
			"lofi hip hop beat, chill, ambient, warm vinyl crackle, mellow piano chords, soft drums, study music, relaxing, instrumental",
		lyrics: "[instrumental]",
		duration: 60,
		bpm: 75,
	},
	rainsound: {
		prompt:
			"gentle rain falling on a window, soft thunder in the distance, ambient rain sounds, nature soundscape, peaceful, calming",
		lyrics: "[instrumental]",
		duration: 60,
	},
	whitenoise: {
		prompt:
			"white noise, soft static, ambient drone, consistent gentle hum, focus sound, minimal, meditative",
		lyrics: "[instrumental]",
		duration: 60,
	},
	ambient: {
		prompt:
			"ambient electronic music, soft synthesizer pads, ethereal textures, floating, spacious, Brian Eno style, instrumental",
		lyrics: "[instrumental]",
		duration: 60,
		bpm: 60,
	},
	classical: {
		prompt:
			"soft classical piano, gentle and contemplative, Erik Satie style, minimalist, peaceful, solo piano instrumental",
		lyrics: "[instrumental]",
		duration: 60,
		bpm: 70,
	},
...
export class ADHDAudio {
	...
	private async generate(soundscape: ADHDSoundscape): Promise<string | null> {
		...
		try {
			...
			const audioUrl = outputPath.startsWith("http")
				? outputPath
				: `${this._config.apiUrl}${outputPath}`;
		...
		while (Date.now() - start < maxWait) {
			...
			try {
				...
				if (res.ok) {
					...
					if (job && job.status === 1) {
						const results = JSON.parse(job.result) as Array<{
							file?: string;
							status?: number;
						}>;
	...
	playFile(filePath: string): { ok: boolean; message: string } {
		...
		return {
			ok: true,
			message: `Playing ${basename(filePath)}. /music stop to end.`,
		};
	}
	...
	async play(soundscape: ADHDSoundscape): Promise<{
		...
		if (!(soundscape in SOUNDSCAPES)) {
			...
			return {
				ok: false,
				message: `Unknown soundscape. Available: ${available}`,
			};
		}
		...
		if (this.hasCached(soundscape)) {
			...
			return {
				ok: true,
				message: `Playing ${soundscape} (${this._config.duration}s loop). /adhd stop to end.`,
			};
		}
