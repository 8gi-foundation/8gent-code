...
export function narrateToolStart(
	...
	switch (toolName) {
		case "run_command": {
			...
			if (cmd === "bun" && String(args.command).includes("test"))
		...
		case "write_file": {
			const writeName = String(args.path || "")
				.split("/")
				.pop();
			return `Writing ${writeName}...`;
		}
		case "edit_file": {
			const editName = String(args.path || "")
				.split("/")
				.pop();
...
export function narrateToolEnd(
	toolName: string,
	success: boolean,
	durationMs: number,
): string {
	...
	switch (toolName) {
		case "run_command":
			return `Command completed (${time})`;
		case "write_file":
			return `File written (${time})`;
		case "read_file":
			return `File loaded (${time})`;
		default:
			return `Done (${time})`;
	}
}
