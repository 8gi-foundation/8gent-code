...
import { getDb, getTemplates, upsertTemplate, getCampaignStats } from "./campaign-db";
    ...
    const res = await fetch(`${MODEL_PROXY_URL}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert LinkedIn outreach copywriter. You write short, direct, non-salesy messages that lead with a real signal about the recipient. Never use cliches. Max 300 chars for connection notes, 500 chars for messages.",
          },
...
function logEvolution(entry: EvolutionEntry): void {
  ...
  `).run(
    entry.id, entry.timestamp, entry.originalTemplateId, entry.newTemplateId,
    entry.reason, entry.originalReplyRate, entry.originalBody, entry.newBody
  );
}

// ── Core reflection logic ─────────────────────────────────────────────

export async function reflect(): Promise<{ evolved: number; skipped: number; details: string[] }> {
  ...
  const underperformers = templates.filter(t =>
    t.sendCount >= REFLECTION_THRESHOLD &&
    t.replyRate < UNDERPERFORM_RATE
  );
...
`.trim();
    ...
    evolved++;
    details.push(`EVOLVED ${template.name}: ${(template.replyRate * 100).toFixed(1)}% → testing v${template.version + 1}`);
...
export function startReflectionLoop(): void {
  ...
  reflectionTimer = setInterval(async () => {
    ...
    console.log(`[hyperagent] Reflection complete. Evolved: ${result.evolved}, Skipped: ${result.skipped}`);
