...
import { searchPeople, getProfile, sendConnectionRequest, sendMessage, getRecentReplies } from "./linkedin-api";
import { enrichLead, buildSignalHook } from "./signal-engine";
import { upsertLead, getLead, getTemplates, getCampaignStats } from "./campaign-db";
...
export const TOOL_DEFINITIONS = [
  {
    ...
    description: "Search LinkedIn for people matching criteria. Returns enriched lead list with buying signals.",
    inputSchema: {
      type: "object",
      properties: {
        keywords: { type: "string", description: "Search keywords, job titles, or company names" },
        titles: { type: "array", items: { type: "string" }, description: "Job title filters" },
        locations: { type: "array", items: { type: "string" }, description: "Location filters" },
        limit: { type: "number", description: "Max results (default 25, max 49)" },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      required: ["publicId"],
      properties: {
        publicId: { type: "string", description: "LinkedIn public identifier (from URL: linkedin.com/in/[this-part])" },
      },
    },
  },
  {
    name: "linkedin_send_connection",
    description: "Send a connection request with a personalized note. Max 300 chars. Requires rate limit budget.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        conversationUrn: { type: "string", description: "LinkedIn conversation URN" },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        since: { type: "string", description: "ISO timestamp - only return replies after this date" },
      },
    },
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        campaignId: { type: "string", description: "Filter to specific campaign (omit for all)" },
      },
    },
  },
  {
  ...
  },
  {
    ...
    description: "Manually trigger HyperAgent reflection loop to evolve underperforming templates now.",
    inputSchema: { type: "object", properties: {} },
  },
  {
  ...
  try {
    switch (call.name) {
      ...
      case "linkedin_get_profile": {
        const publicId = (args.publicId as string).replace("https://www.linkedin.com/in/", "").replace(/\/$/, "");
        const profile = await getProfile(publicId);
        const lead = { id: randomId(), signals: [], connectionDegree: 3 as const, ...profile } as any;
        ...
        return text(JSON.stringify({
          ...
          suggestedOpener: buildSignalHook(enriched) || "No strong signal found - consider skipping",
        }, null, 2));
      }

      case "linkedin_send_connection": {
        if (!limiter.canSend("connection_requests")) {
          return error(`Daily connection request cap reached (${limiter.remaining("connection_requests")} remaining). Try tomorrow.`);
        }

        const { profileUrn, note } = args as { profileUrn: string; note: string };
        ...
        return text(`Connection request queued for ${profileUrn}. Remaining today: ${limiter.remaining("connection_requests")}`);
      }
      ...
      case "linkedin_send_message": {
        ...
        const { conversationUrn, body } = args as { conversationUrn: string; body: string };
      ...
      case "linkedin_get_rate_status": {
        ...
        const lines = Object.entries(status).map(
          ([action, s]: [string, any]) => `${action}: ${s.used}/${s.cap} used (${s.remaining} remaining)`
        );
